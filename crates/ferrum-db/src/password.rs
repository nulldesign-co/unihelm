...
pub fn hash_password(password: &str) -> Result<String, FerrumError> {
    check_strength(password)?;
    let salt = SaltString::generate(&mut rand::thread_rng());
    argon2()
        .hash_password(password.as_bytes(), &salt)
        .map(|h| h.to_string())
        .map_err(|e| FerrumError::internal(format!("password hashing failed: {e}")))
}
...
pub fn check_strength(password: &str) -> Result<(), FerrumError> {
    const MIN: usize = 12;
    ...
    if password.len() > MAX {
        return Err(
            FerrumError::new(ErrorCode::PasswordTooWeak, "password is too long")
                .with_field("password"),
        );
    }
    Ok(())
