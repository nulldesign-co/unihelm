...
use ferrum_core::{ErrorCode, FerrumError, PlanId, SubscriptionId, TenantScope, UserId};
use serde::Serialize;
    ...
    pub async fn create(&self, new: NewPlan) -> Result<Plan> {
        let owner = match self.scope {
            ...
            ScopeFilter::Customer(_) | ScopeFilter::Subscription { .. } => {
                return Err(DbError::Domain(FerrumError::denied(
...
mod tests {
    ...
    use ferrum_core::{Domain, Email, Role, SiteId, Username};
