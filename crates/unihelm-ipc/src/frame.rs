...
use unihelm_core::{AuthContext, TaskId, UnihelmError, UserId};
use uuid::Uuid;
...
impl ResponseFrame {
    ...
    pub fn err(id: Uuid, error: UnihelmError) -> Self {
        Self {
            v: PROTOCOL_VERSION,
            id,
...
/// (`unihelm_ops::terminal::authorize`). A frame asking for `Root` from a
...
pub enum ControlKind {
    ...
    /// the AC in spec §11.16: the shell survives a `unihelm-web` restart.
    TerminalAttach {
        session: Uuid,
        auth: unihelm_core::AuthContext,
    },
    ...
    /// It is not decoration: one `unihelm-web` process multiplexes every
...
mod tests {
    ...
    fn response_and_event_are_unambiguous() {
        ...
        for f in [
            ...
            ServerFrame::Response(ResponseFrame::err(id, UnihelmError::invalid("nope"))),
