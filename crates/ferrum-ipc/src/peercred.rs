...
//! Filesystem permissions (0700, owned by the `ferrum` user) are the first lock;
...
pub struct PeerPolicy {
    /// uids allowed to connect — normally just the `ferrum` web user.
    pub allowed_uids: Vec<u32>,
    /// root is always allowed: the CLI and `ferrum doctor` run as root, and a
...
impl PeerPolicy {
    ...
    ///
    /// Never constructed by the packaged agent — `ferrum-agentd` resolves the
    /// real `ferrum` uid at startup and refuses to run without it.
