...
//!
//! Everything Unihelm writes sits under a `unihelm.d` directory of its own. We
...
pub fn nginx_hook() -> PathBuf {
    under("/etc/nginx/conf.d/unihelm.conf")
}
...
pub fn site_log_root() -> PathBuf {
    under("/var/log/unihelm/sites")
}

/// The panel's data directory.
pub fn data_dir() -> PathBuf {
    under("/var/lib/unihelm")
}

/// `/etc/unihelm` — `config.toml` and the master key (spec §4.3).
///
...
pub fn cert_root() -> PathBuf {
    under("/var/lib/unihelm/state/certs")
}
...
pub fn logrotate_site(domain: &str) -> PathBuf {
    under("/etc/logrotate.d").join(format!("unihelm-{domain}"))
}
...
pub fn fpm_pool_file(family: Family, version: PhpVersion, site: &str) -> PathBuf {
    fpm_pool_dir(family, version).join(format!("unihelm-{site}.conf"))
}
...
/// A unit file the panel owns, by its full file name (`unihelm-uh_ab.slice`).
...
pub fn sshd_dropin() -> PathBuf {
    under("/etc/ssh/sshd_config.d/50-unihelm.conf")
}
...
/// nginx's own top-level configuration. Unihelm never writes it; the WAF
/// preflight *reads* it to find out whether it offers a main-context `include`
/// that a `load_module` line could live in (see `unihelm_ops::waf`).
...
/// The WAF's configuration directory: everything ModSecurity reads that Unihelm
/// generates.
...
/// Rule Set and carries the per-site policy Unihelm renders from the database.
...
pub fn waf_audit_log() -> PathBuf {
    under("/var/log/unihelm/waf/audit.log")
}

/// Where the per-site mail relay configuration lives (spec §11.18).
///
/// Under `/etc/unihelm` rather than in the tenant home: the file holds the
...
/// One plugin's installed tree: `/var/lib/unihelm/plugins/<slug>`.
...
pub fn plugin_runtime_dir(slug: &str) -> PathBuf {
    under("/run/unihelm/plugins").join(slug)
}
...
pub fn plugin_runtime_dir_relative(slug: &str) -> String {
    format!("unihelm/plugins/{slug}")
}

/// The unit file name for one plugin's sidecar.
pub fn plugin_unit_file_name(slug: &str) -> String {
    format!("unihelm-plugin-{slug}.service")
}

/// `/etc/systemd/system/unihelm-plugin-<slug>.service`.
...
mod tests {
    ...
    fn every_waf_file_the_panel_writes_is_inside_a_directory_it_owns() {
        ...
        // editing nginx.conf. Nothing here may land outside `unihelm.d`,
        // `/etc/unihelm` or `/var/lib/unihelm` (spec §10.4 rule 1).
    ...
    fn the_paths_the_posture_scan_reads_are_never_written_by_the_panel() {
        ...
        // reads Unihelm's chrooted-SFTP block too — as it should, since that
    ...
    fn site_paths_follow_the_documented_layout() {
        assert_eq!(
            site_public("uh_abc123", "example.com").to_str().unwrap(),
            "/home/uh_abc123/sites/example.com/public"
        );
    }
