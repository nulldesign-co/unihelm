...
use unihelm_core::{AuthContext, ErrorCode, TaskId, UnihelmError, UserId};
use unihelm_db::tasks::NewTask;
use unihelm_ipc::frame::{
    ControlFrame, ControlKind, EventFrame, EventKind, RequestFrame, ResponseBody, TerminalTarget,
};
use unihelm_ipc::peercred::PeerCred;
use unihelm_ipc::server::{EventSink, RequestHandler};
use unihelm_ops::terminal::{SessionHandle, TerminalRegistry};
use unihelm_ops::{Execution, OpRegistry};
use uuid::Uuid;
...
pub struct Agent {
    ...
    /// than on a connection so a session outlives the `unihelm-web` process that
...
pub struct ConnectionHandler {
    ...
    /// shell after `unihelm-web` restarts (spec 11.16).
    ///
    /// Shared with the pumps so each can drop its own entry when its session
    /// ends. `unihelm-web` holds one IPC connection for its whole process life,
...
impl ConnectionHandler {
    ...
    /// `unihelm_ops::terminal` decide what that identity may have.
    ///
    ...
    /// a `unihelm-web` restart or a dropped WebSocket (spec 11.16 AC).
    ...
    ///
    /// The actor check is the load-bearing half. One `unihelm-web` process
