...
//!
//!     cargo run -p unihelm-distro --example verify-pins
//!
...
use unihelm_distro::detect::{Arch, DistroInfo, Family};
use unihelm_distro::pgp;
...
async fn main() {
    ...
    for (label, info) in &targets {
        println!("\n=== {label} ===");
        for repo in unihelm_distro::repos::catalogue(info) {
