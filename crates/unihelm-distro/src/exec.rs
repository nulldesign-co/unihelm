...
/// re-execs itself as `unihelm-agentd --fs-helper ...`, which drops to the
...
impl From<DistroError> for UnihelmError {
...
mod tests {
    ...
    async fn arguments_are_never_interpreted_by_a_shell() {
        ...
        let payload = "a; touch /tmp/unihelm-pwned; `id`; $(id); a|b>c";
        let out = Cmd::new("echo").arg(payload).run().await.unwrap();
        assert_eq!(out.trimmed_stdout(), payload);
        assert!(!std::path::Path::new("/tmp/unihelm-pwned").exists());
    }
    ...
    async fn environment_is_scrubbed_by_default() {
        ...
        unsafe { std::env::set_var("UNIHELM_TEST_LEAK", "secret") };
        let out = Cmd::new("env").run().await.unwrap();
        assert!(
            !out.stdout.contains("UNIHELM_TEST_LEAK"),
            "parent env must not leak into commands"
        );
        let out = Cmd::new("env").inherit_env().run().await.unwrap();
        assert!(out.stdout.contains("UNIHELM_TEST_LEAK"));
    }
}
