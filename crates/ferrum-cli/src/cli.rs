...
pub enum Command {
    ...
    /// subcommand so `ferrum completions bash > …` works anywhere.
    #[command(hide = true)]
    Completions {
...
pub enum SiteCommand {
    ...
    Update {
        /// The site's id, as `ferrum site list` shows it.
        site_id: i64,
...
pub enum DbCommand {
    ...
    Drop {
        /// The database's id, as `ferrum db list` shows it.
        database_id: i64,
...
pub enum DbUserCommand {
    ...
    Drop {
        /// The full user name, as `ferrum db list` shows it.
        username: String,
    },
...
pub enum BackupCommand {
    ...
    /// Take a backup now.
    Run {
        /// Repository id, as `ferrum backup repo` rows show it.
        #[arg(long)]
        repo: i64,
...
pub enum CronCommand {
    ...
    /// Delete a job.
    Delete {
        /// Job id, as `ferrum cron list` shows it.
        id: i64,
    },
}
...
pub enum DnsCommand {
    ...
    ProviderSet {
        ...
        #[arg(long)]
        label: String,
        /// Read the API token from stdin. Falls back to `FERRUM_DNS_TOKEN`.
        #[arg(long)]
        token_stdin: bool,
    },
...
pub enum AppCommand {
    ...
    Delete {
        /// Application id, as `ferrum app list` shows it.
        app_id: i64,
    },
...
pub enum WordpressCommand {
    ...
    Update {
        /// Install id, as `ferrum wp detect` and the site listing show it.
        install_id: i64,
...
pub enum PlanCommand {
    ...
    Update {
        /// Plan id, as `ferrum plan list` shows it.
        plan_id: i64,
        /// Rename the plan.
        #[arg(long)]
        name: Option<String>,
...
pub enum SubscriptionCommand {
    ...
    Suspend {
        /// Subscription id, as `ferrum subscription list` shows it.
        subscription_id: i64,
...
pub enum AlertCommand {
    ...
    ChannelsDelete {
        /// Channel id, as `ferrum alert channels` shows it.
        id: i64,
    },
...
mod tests {
    ...
    fn json_and_follow_are_global_so_they_work_after_the_subcommand() {
        // `ferrum site list --json` must parse: a scripting flag that only works
        // before the subcommand is a flag people file bugs about.
        let cli = Cli::try_parse_from(["ferrum", "site", "list", "--json"]).unwrap();
        assert!(cli.json);
        let cli = Cli::try_parse_from(["ferrum", "task", "logs", "abc", "-f"]).unwrap();
        assert!(cli.follow);
    }
    ...
    fn a_secret_can_never_be_given_as_an_argument() {
        ...
        for argv in [
            ...
            vec!["ferrum", "sftp", "enable", "1", "--password", "secret"],
        ] {
            assert!(
