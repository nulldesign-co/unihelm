...
//! honest way to check that `ferrum site create` sends what `site.create`
...
fn unit_value(unit: UnitArg, version: Option<&str>) -> Result<Value> {
    let name = match unit {
        ...
        UnitArg::Sshd => "sshd",
        UnitArg::FerrumWeb => "ferrum_web",
        UnitArg::FerrumAgentd => "ferrum_agentd",
    };
...
mod tests {
    ...
    fn site_create_sends_only_the_fields_that_were_given() {
        let inv = invocation(&["ferrum", "site", "create", "example.com"]);
    ...
    fn php_install_targets_the_stack_manager() {
        let inv = invocation(&["ferrum", "php", "install", "8.3", "--ext", "redis,imagick"]);
    ...
    fn a_php_component_without_a_version_is_refused_before_the_agent_is_told() {
        let err = plan_argv(&["ferrum", "stack", "install", "php"]).unwrap_err();
        assert!(err.to_string().contains("--version"), "{err}");
    }

    #[test]
    fn dropping_a_database_carries_the_retyped_name() {
        let inv = invocation(&["ferrum", "db", "drop", "3", "--confirm-name", "shop"]);
    ...
    fn php_fpm_is_the_one_unit_that_needs_a_version() {
        let inv = invocation(&["ferrum", "svc", "status", "maria-db"]);
        assert_eq!(inv.input["unit"], json!({ "unit": "maria_db" }));

        let err = plan_argv(&["ferrum", "svc", "status", "php-fpm"]).unwrap_err();
        assert!(err.to_string().contains("--version"), "{err}");

        let inv = invocation(&["ferrum", "svc", "status", "php-fpm", "--version", "8.3"]);
        assert_eq!(
            inv.input["unit"],
    ...
    fn changing_one_sentinel_knob_reads_the_rest_back_first() {
        ...
        let action =
            plan_argv(&["ferrum", "firewall", "settings-set", "--enabled", "true"]).unwrap();
    ...
    fn an_empty_allowlist_flag_clears_the_list() {
        let action = plan_argv(&["ferrum", "firewall", "settings-set", "--allowlist", ""]).unwrap();
        let Action::MergeSentinelSettings(patch) = action else {
    ...
    fn wildcards_go_through_the_dns_01_operation_not_the_http_01_one() {
        let inv = invocation(&["ferrum", "dns", "issue-wildcard", "5", "--staging"]);
        assert_eq!(inv.op, "cert.issue_wildcard");
        assert_eq!(inv.input["staging"], true);

        let inv = invocation(&["ferrum", "cert", "issue", "5"]);
    ...
    fn local_commands_never_reach_the_agent() {
        for argv in [
            vec!["ferrum", "doctor"],
            vec!["ferrum", "user", "list"],
            vec!["ferrum", "task", "list"],
