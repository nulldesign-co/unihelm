...
pub mod keys {
    ...
    //
    // Defaults live in `unihelm_ops::fwops::SentinelSettings`, not here, because
    ...
    //
    // Same reasoning as Sentinel's keys: defaults live in `unihelm_ops::waf`
    ...
    /// `unihelm_ops::waf` is visible without hashing the tree again.
...
mod tests {
    ...
    async fn settings_round_trip_and_upsert() {
        ...
        db.set_setting(keys::PANEL_NAME, &"Unihelm".to_string())
            .await
            .unwrap();
        assert_eq!(
            db.get_setting::<String>(keys::PANEL_NAME).await.unwrap(),
            Some("Unihelm".to_string())
        );
