...
impl Db {
    ...
    /// quota that is not actually live (the caller in `unihelm-ops` owns that
    /// ordering).
...
mod tests {
    ...
    async fn each_subscription_gets_its_own_project_id() {
        ...
        let a = db.quota_project_for(subs[0], "/home/uh_a").await.unwrap();
        let b = db.quota_project_for(subs[1], "/home/uh_b").await.unwrap();
        let c = db.quota_project_for(subs[2], "/home/uh_c").await.unwrap();
    ...
    async fn allocation_is_idempotent_per_subscription() {
        let (db, subs) = seed().await;
        let first = db.quota_project_for(subs[0], "/home/uh_a").await.unwrap();
        let again = db.quota_project_for(subs[0], "/home/uh_a").await.unwrap();
    ...
    async fn a_deleted_tenants_project_id_is_reused_not_leaked() {
        let (db, subs) = seed().await;
        db.quota_project_for(subs[0], "/home/uh_a").await.unwrap();
        let b = db.quota_project_for(subs[1], "/home/uh_b").await.unwrap();
        db.quota_project_for(subs[2], "/home/uh_c").await.unwrap();
        ...
        let reused = db.quota_project_for(sub.id, "/home/uh_d").await.unwrap();
    ...
    async fn the_first_id_itself_is_reusable_after_deletion() {
        let (db, subs) = seed().await;
        let a = db.quota_project_for(subs[0], "/home/uh_a").await.unwrap();
        db.quota_project_for(subs[1], "/home/uh_b").await.unwrap();
        assert_eq!(a.project_id, FIRST_PROJECT_ID);
        ...
        let again = db.quota_project_for(subs[2], "/home/uh_c").await.unwrap();
