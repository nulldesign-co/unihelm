...
//! human-readable reason. `unihelm-agentd` is the only writer here (spec §5.5).

use unihelm_core::{ErrorCode, SubscriptionId, TaskId, TenantScope, UnihelmError, UserId};
...
impl Db {
    ...
    pub async fn finish_task_failed(&self, id: TaskId, error: &UnihelmError) -> Result<()> {
        sqlx::query(
...
mod tests {
    ...
    async fn the_history_filters_by_op_status_and_date_without_widening_the_scope() {
        ...
        db.finish_task_failed(
            mine_failed.id,
            &UnihelmError::new(ErrorCode::CommandFailed, "nginx said no"),
        )
        .await
        .unwrap();
        let theirs = db
    ...
    async fn a_finished_task_is_not_reopened_by_a_late_update() {
        ...
        db.finish_task_failed(t.id, &UnihelmError::internal("too late"))
            .await
            .unwrap();
        let after = db
            .tasks(&TenantScope::Global)
