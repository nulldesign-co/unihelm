//! `unihelm` — the command-line half of the panel (spec §11.20).
//!
...
//! agent socket `unihelm-web` opens, and the agent re-derives the acting
...
//!
//! - **`unihelm doctor`**, the first thing to run when something looks wrong. It
//!   checks the pieces in the order they depend on each other, so the first
//!   failure is usually the real one.
//! - **`unihelm user create-admin`**, which the installer calls to create the
//!   first account, before any session, listener or agent exists.
//! - **`unihelm task …`**, which reads the task table directly; see `tasks.rs`.
//!
...
fn load_config(cli: &Cli) -> Result<UnihelmConfig> {
...
fn report_failure(error: &anyhow::Error, json: bool) -> i32 {
    if let Some(TransportFailure(unihelm)) = error.downcast_ref::<TransportFailure>() {
        if json {
            print_json(&serde_json::json!({
                "error": {
                    "code": unihelm.code.code(),
                    "slug": unihelm.code.slug(),
                    "detail": unihelm.detail,
                }
            }));
        } else {
        ...
        return session::exit_code_for(unihelm.code);
    }
...
fn resolve_secrets(command: &Command) -> Result<Secrets> {
    ...
    match command {
        Command::Dns(DnsCommand::ProviderSet { token_stdin, .. }) => {
            secrets.dns_token = secret(*token_stdin, "UNIHELM_DNS_TOKEN")?;
        }
        Command::Backup(BackupCommand::Repo(BackupRepoCommand::Init {
            s3_secret_stdin, ..
        })) => {
            secrets.s3_secret_access_key =
                secret(*s3_secret_stdin, "UNIHELM_S3_SECRET_ACCESS_KEY")?;
        }
        ...
        Command::Mail(MailCommand::Relay(MailRelayCommand::Set { password_stdin, .. })) => {
            secrets.mail_relay_password =
                secret_allowing_empty(*password_stdin, "UNIHELM_MAIL_RELAY_PASSWORD", true)?;
        }
        _ => {}
    }
    Ok(secrets)
...
async fn doctor(config: &UnihelmConfig, json: bool) -> Result<i32> {
...
async fn check_agent(config: &UnihelmConfig, report: &mut Report) {
    ...
    if !socket.exists() {
        report.fail(
            "agent socket",
            format!(
                "{} does not exist — is unihelm-agentd running?",
                socket.display()
            ),
        );
        return;
...
fn check_disk(config: &UnihelmConfig, report: &mut Report) {
    let dir = config
        .panel
        .database
        .parent()
...
mod tests {
    ...
    fn a_secret_flag_prefers_stdin_but_falls_back_to_the_environment() {
        ...
        unsafe { std::env::set_var("UNIHELM_TEST_SECRET", "from-env") };
        assert_eq!(
            secret(false, "UNIHELM_TEST_SECRET").unwrap(),
            Some("from-env".to_string())
        );
        unsafe { std::env::set_var("UNIHELM_TEST_SECRET", "") };
        assert_eq!(
            secret(false, "UNIHELM_TEST_SECRET").unwrap(),
            None,
            "an empty variable is not a credential"
        );
        unsafe { std::env::remove_var("UNIHELM_TEST_SECRET") };
        assert_eq!(secret(false, "UNIHELM_TEST_SECRET").unwrap(), None);
    }

    #[test]
    fn a_command_with_no_secret_resolves_none_and_never_reads_stdin() {
        // If this ever blocked, `unihelm site list` in a script with no stdin
        // would hang for ever.
        let cli = Cli::try_parse_from(["unihelm", "site", "list"]).unwrap();
