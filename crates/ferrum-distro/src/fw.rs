...
const MARK: &str = "ferrum";

/// The nftables table we own outright.
const NFT_TABLE: &str = "ferrum";

/// Ban sets. Two of them, because a `hash:ip` set holds one address family.
const BAN_SET_V4: &str = "ferrum_bans";
const BAN_SET_V6: &str = "ferrum_bans6";
...
pub trait FwBackend: Send + Sync {
    ...
    /// The rules Ferrum manages. Not the whole ruleset — the operator's own
...
/// rule never shows up in the panel as something Ferrum owns and may delete.
...
/// Read Ferrum's rules back out of `ufw status verbose`.
///
/// Lines look like:
/// `80/tcp                     ALLOW IN    Anywhere                   # ferrum: http`
...
/// nftables directly, in a dedicated `inet ferrum` table so we never edit rules
/// somebody else owns.
///
...
/// host where Ferrum owns the policy. Bans are unaffected: `drop` **is**
...
/// reported as Ferrum's to delete.
...
mod tests {
    ...
    fn someone_elses_rich_rule_is_not_reported_as_ours() {
        ...
        for foreign in [
            ...
            r#"rule source ipset="ferrum_bans" drop"#,
            "",
        ] {
            assert!(
                parse_rich_rule(foreign).is_none(),
                "{foreign} should not be read as a Ferrum port rule"
            );
        }
    }
...
80/tcp                     ALLOW IN    Anywhere                   # ferrum: http
443/tcp                    ALLOW IN    Anywhere                   # ferrum: https
3306/tcp                   ALLOW IN    10.0.0.0/8                 # ferrum: remote mysql
...
table inet ferrum {
  chain input {
    type filter hook input priority -5; policy accept;
    ip saddr @ferrum_bans drop
    tcp dport 80 accept comment "ferrum: http"
    ip saddr 10.0.0.0/8 tcp dport 3306 accept comment "ferrum: remote mysql"
