...
//!
//! A thin bridge onto the `cron.*` operations in `unihelm_ops::cron`. The agent
...
pub struct CreateRequest {
    ...
    #[schema(example = "/usr/bin/php /home/uh_ab12cd34/cron.php")]
    pub command: String,
...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 404, description = "`not_found`: no such subscription in this tenant's scope", body = ApiErrorBody),
        (status = 409, description = "`conflict`: the account already has a crontab Unihelm did not write, or is at its job limit", body = ApiErrorBody),
        (status = 503, description = "`agent_unavailable`", body = ApiErrorBody),
    ),
)]
pub async fn create(
...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 409, description = "`conflict`: the account has a crontab Unihelm did not write", body = ApiErrorBody),
        (status = 503, description = "`agent_unavailable`", body = ApiErrorBody),
    ),
)]
pub async fn update(
