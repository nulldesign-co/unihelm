...
//!
//! A thin bridge onto the `wp.*` operations in `unihelm_ops::wordpress`. The
...
pub async fn detect(
    ...
    if let Some(sub) = &q.subdirectory {
        let parsed = unihelm_core::TenantPath::parse(sub)
...
fn install_input(body: &InstallRequest) -> ApiResult<serde_json::Value> {
    ...
    // trusted.
    let email = unihelm_core::Email::parse(&body.admin_email)
        .map_err(|e| ApiError::new(e.with_field("admin_email")))?;
    let admin_user = unihelm_core::Username::parse(&body.admin_user)
...
mod tests {
    ...
    fn the_admin_user_that_reaches_the_operation_is_the_parsed_one() {
        ...
        assert_eq!(
            input["admin_user"],
            json!(unihelm_core::Username::parse("SiteAdmin").unwrap().as_str())
        );
    }
}
