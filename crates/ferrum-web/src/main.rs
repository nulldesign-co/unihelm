//! `ferrum-web` — the unprivileged half of the panel (spec §5.1).
//!
...
//! files. Everything privileged crosses the Unix socket into `ferrum-agentd`,
...
#[derive(Parser, Debug)]
#[command(name = "ferrum-web", version, about = "Ferrum panel web server")]
struct Args {
...
async fn main() -> Result<()> {
    ...
    if is_root() {
        anyhow::bail!(
            "refusing to run as root: ferrum-web is the unprivileged half of the panel \
             and must run as the `ferrum` user (spec §12 rule 1)"
        );
    }
    ...
    if !db.has_any_user().await? {
        tracing::warn!(
            "no accounts exist yet — create the first administrator with `ferrum user create-admin`"
        );
    }
    ...
    tracing::info!(%addr, "ferrum-web listening");
...
fn install_panic_hook() {
    ...
    std::panic::set_hook(Box::new(move |info| {
        tracing::error!(panic = %info, "ferrum-web panicked");
        default(info);
    }));
}
