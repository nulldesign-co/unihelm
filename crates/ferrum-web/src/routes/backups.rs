...
//!
//! Thin over `ferrum_ops::backup`, like every route module — the agent
...
//!    architecture note is in `ferrum_ops::backup::RepoInit`; the consequence
...
use ferrum_core::{ErrorCode, Permission, TenantScope};
...
///
/// The mirror of `ferrum_ops::backup::require_global`, applied here as well so
...
/// password: `ferrum_db::BackupRepo` has no field for either.
#[utoipa::path(
    get,
...
pub struct RepoInitRequest {
    ...
    #[schema(example = "s3.example.com/ferrum-backups")]
...
/// this server together with `/etc/ferrum/secret.key`; without both, a
...
/// `/etc/ferrum` and the state directory; subscription scope writes the tenant
/// home.
#[utoipa::path(
    post,
    path = "/api/backups/runs",
...
mod tests {
    ...
    fn a_retention_field_the_caller_omitted_is_absent_rather_than_null() {
        ...
        // `#[serde(default = "…")]` in `ferrum_ops::backup::ScheduleSetInput`.
