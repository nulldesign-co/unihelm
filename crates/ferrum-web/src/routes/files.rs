...
use ferrum_core::{ErrorCode, FerrumError, Permission, TenantPath};
...
/// [`ferrum_ipc::codec::MAX_FRAME_BYTES`]. A client that wants to move more
...
const _: () = assert!(
    (MAX_CONTENT_BYTES / 3 + 1) * 4 + 64 * 1024 < ferrum_ipc::codec::MAX_FRAME_BYTES,
...
pub(crate) fn parse_read_chunk(data: &Value) -> Result<ReadChunk, FerrumError> {
    let content = data
        .get("content_b64")
        .and_then(Value::as_str)
        .ok_or_else(|| {
            FerrumError::new(ErrorCode::AgentProtocol, "fs.read reply has no content_b64")
        })?;
    let eof = data.get("eof").and_then(Value::as_bool).ok_or_else(|| {
        FerrumError::new(ErrorCode::AgentProtocol, "fs.read reply has no eof flag")
    })?;
...
pub(crate) fn chunk_stream<F, Fut>(
    first: ReadChunk,
    fetch: F,
) -> impl Stream<Item = Result<axum::body::Bytes, FerrumError>> + Send
where
    F: Fn(u64) -> Fut + Send + 'static,
    Fut: Future<Output = Result<ReadChunk, FerrumError>> + Send,
{
...
/// wrong.
pub(crate) fn ensure_downloadable(stat: &Value) -> Result<(), FerrumError> {
...
pub(crate) fn checked_content_len(content_b64: &str) -> Result<u64, FerrumError> {
...
pub(crate) fn regular_file_size(stat: &Value) -> Result<u64, FerrumError> {
    let entry = stat.get("entry").filter(|v| v.is_object()).unwrap_or(stat);
    match entry.get("kind").and_then(Value::as_str) {
        Some("file") => {}
        Some(_) => {
    ...
    entry
        ...
        .ok_or_else(|| FerrumError::new(ErrorCode::AgentProtocol, "fs.stat reply has no size"))
}
...
pub(crate) fn validate_archive_entry(name: &str) -> Result<(), FerrumError> {
...
pub async fn compress(
    ...
    if body.entries.is_empty() {
        return Err(ApiError::new(
            FerrumError::new(ErrorCode::InvalidInput, "nothing selected to compress")
                .with_field("entries"),
        ));
    }
...
pub async fn trash_purge(
    ...
    if body.older_than_days > MAX_PURGE_DAYS {
        return Err(ApiError::new(
            FerrumError::new(ErrorCode::InvalidInput, "older_than_days is out of range")
...
mod tests {
    ...
    fn scripted_fetch(
        script: Vec<Result<ReadChunk, FerrumError>>,
        offsets: Arc<std::sync::Mutex<Vec<u64>>>,
    ) -> impl Fn(
        u64,
    )
        -> std::pin::Pin<Box<dyn Future<Output = Result<ReadChunk, FerrumError>> + Send>>
    + Send
    + 'static {
    ...
    async fn a_mid_stream_error_ends_the_stream_instead_of_truncating_silently() {
        let offsets = Arc::new(std::sync::Mutex::new(Vec::new()));
        let fetch = scripted_fetch(
            vec![Err(FerrumError::new(ErrorCode::NotFound, "vanished"))],
            offsets.clone(),
        );
    ...
    async fn panel() -> Panel {
        ...
        let user = db
            ...
            .unwrap();
        let issued = db
            .create_session(user.id, None, None, ferrum_db::sessions::DEFAULT_TTL, None)
            .await
            .unwrap();

        let mut config = ferrum_core::FerrumConfig::default();
        // Point the agent socket somewhere that certainly does not exist.
        config.agent.socket = std::path::PathBuf::from("/nonexistent/ferrum-agent.sock");
