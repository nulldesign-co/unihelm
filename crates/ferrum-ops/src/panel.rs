...
//!
//! `ferrum-web` listens on loopback and never faces the internet directly;
...
use ferrum_core::{Domain, Email, ErrorCode, FerrumError, Permission, Result};
use ferrum_db::CertKind;
...
/// inside `ferrum-web`, so this is a ceiling, not an invitation.
const PANEL_MAX_BODY: &str = "64m";

/// Where `ferrum-web` actually listens, for the vhost's `proxy_pass`.
pub fn web_upstream() -> String {
    web_upstream_from(Path::new(ferrum_core::config::paths::CONFIG))
}

/// The agent and the web process read the same `/etc/ferrum/config.toml`, so
...
fn web_upstream_from(path: &Path) -> String {
    let fallback = FerrumConfig::default().panel.listen;
    let Ok(text) = std::fs::read_to_string(path) else {
        return fallback;
    };
    match FerrumConfig::from_toml(&text) {
...
impl TypedOperation for Issue {
    ...
    async fn run(&self, ctx: &OpContext, input: Self::Input) -> Result<Self::Output> {
        ...
        db.set_setting(ferrum_db::panel::DOMAIN_KEY, &domain)
            .await
...
mod tests {
    ...
    fn render_panel(domain: &str, upstream: &str) -> String {
        let set = ferrum_config::TemplateSet::load().unwrap();
        set.render(
            "nginx/panel.conf",
    ...
    fn the_panel_vhost_proxies_to_the_web_listen_address() {
        ...
        assert!(
            out.contains("/var/lib/ferrum/state/certs/panel.example.com/fullchain.pem"),
            "certificate path must follow the shared cert store layout:\n{out}"
        );
        assert!(
            out.contains("/var/lib/ferrum/state/certs/panel.example.com/privkey.pem"),
            "{out}"
        );
    ...
    fn the_http_server_keeps_acme_reachable_and_redirects_everything_else() {
        ...
        assert!(out.contains("root /var/lib/ferrum/state/acme;"), "{out}");
        assert!(
    ...
    fn a_missing_config_file_falls_back_to_the_loopback_default() {
        ...
        // /etc; the fallback must match ferrum-web's own default listen.
        assert_eq!(
            web_upstream_from(Path::new("/nonexistent/ferrum/config.toml")),
            "127.0.0.1:8088"
        );
    }
