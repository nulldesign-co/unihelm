...
pub use ferrum_db::imports::ImportSource;
...
pub struct ImportPlan {
    ...
    /// The Ferrum subscription every mapped object will be created under.
...
pub struct PlannedDatabase {
    ...
    /// What Ferrum will call it. Different from `source_name` whenever the
    /// original is not a valid [`ferrum_core::DbName`] or is already taken on
    /// this server — the rename is in the plan precisely so nobody discovers it
    /// afterwards by reading a connection error.
    pub target_name: String,
    /// The database user Ferrum creates and grants on it. A **new** user with a
...
pub enum UnmappedKind {
    ...
    Site,
    /// Source-panel state with no Ferrum equivalent: bandwidth counters,
...
/// Turn a source database (or user) name into one Ferrum will accept.
///
/// cPanel and aaPanel both allow names Ferrum's [`ferrum_core::DbName`] does
...
mod tests {
    use super::*;
    use ferrum_core::DbName;

    #[test]
    fn source_database_names_map_onto_names_ferrum_accepts() {
        for source in [
            "bob_wp",
            "bob-wp",
            "9lives",
