...
//! certificates and a pile of cPanel bookkeeping. Ferrum can host the first
    ...
    for (domain, role) in planned {
        let Ok(parsed) = Domain::parse(&domain) else {
            plan.unmapped(
                UnmappedKind::Site,
                domain,
                "not a domain name Ferrum will serve",
            );
            continue;
        };
        ...
        if detected.is_some() && detected_php.is_none() {
            plan.note(format!(
                "{domain}: the archive says PHP `{}`, which Ferrum does not offer; it will be \
                 created with {}",
            ...
            plan.unmapped(
                ...
                format!(
                    "its dump is {size} bytes, past the {} byte limit this importer loads in one \
                     piece; create the database in Ferrum and restore the dump with the MariaDB \
                     client instead",
...
fn report_unmappables(plan: &mut ImportPlan, index: &TarIndex, top: &str, account: Option<&str>) {
    ...
    for address in accounts.iter().take(MAX_LISTED) {
        plan.unmapped(
            UnmappedKind::Mail,
            address.clone(),
            "Ferrum v1 has no mail server (spec §11.18 ships relay-only); the mailbox and its \
    ...
    if mail_files > 0 {
        plan.unmapped(
            ...
            "stored mail is not imported. Keep this archive: it is the only copy of those \
             messages Ferrum will ever have",
        );
    }
    ...
    for name in &index.names {
        ...
        if let Some(zone) = rest.strip_prefix("dnszones/") {
            plan.unmapped(
                ...
                "Ferrum is not an authoritative nameserver (spec §11.13 manages Cloudflare \
            ...
            plan.unmapped(
                ...
                cert,
                "certificates are not imported; Ferrum issues its own from Let's Encrypt once \
            ...
            plan.unmapped(
                UnmappedKind::Mail,
                rest,
                "Mailman lists have no equivalent in Ferrum",
            );
        } else if rest.starts_with("va/") {
            plan.unmapped(
                UnmappedKind::Mail,
                rest,
                "vacation/autoresponder settings have no equivalent in Ferrum",
            );
        }
    }
    ...
    if let Some(account) = account {
        plan.unmapped(
            ...
            "no password or password hash is imported. Ferrum creates a new database user with a \
...
mod tests {
    ...
    fn php_versions_are_read_from_the_shapes_cpanel_writes() {
        ...
        // A version Ferrum does not offer must not be silently mapped onto one
        // that it does.
