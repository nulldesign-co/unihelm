...
use ferrum_core::{ErrorCode, FerrumError, Permission, Result, Role, TenantScope};
use ferrum_db::branding::PANEL_DEFAULT;
use ferrum_db::{AssetKind, BrandingField, BrandingUpdate, ImageType, ResolvedBranding};
...
pub fn sniff_image(bytes: &[u8]) -> Result<ImageType> {
    let refuse = |detail: String| {
        Err::<ImageType, FerrumError>(
            FerrumError::new(ErrorCode::InvalidInput, detail).with_field("content_b64"),
        )
    };
    ...
    if looks_like_markup(bytes) {
        return refuse(
            "that file is XML or HTML — most likely an SVG. Ferrum does not accept SVG for \
...
pub fn parse_login_host(input: &str) -> Result<String> {
    let value = ferrum_db::branding::normalize_login_host(input);
    let invalid = |detail: &str| {
        FerrumError::new(ErrorCode::InvalidInput, detail.to_string()).with_field("login_host")
    };
    ...
    match ctx.auth().acting_role {
        ...
        Role::Reseller => {
            ...
            match requested {
                ...
                Some(_) => Err(FerrumError::not_found("branding")),
            }
        }
        Role::Customer => Err(FerrumError::new(
            ErrorCode::PermissionDenied,
...
pub struct GetOutput {
    ...
    pub own: Option<ferrum_db::Branding>,
...
impl TypedOperation for Get {
    ...
    async fn run(&self, ctx: &OpContext, input: Self::Input) -> Result<Self::Output> {
        ...
        Ok(GetOutput {
            reseller_id,
            own: db.branding(reseller_id).await.map_err(FerrumError::from)?,
            resolved: db
...
mod tests {
    ...
    async fn a_reseller_cannot_write_another_resellers_branding() {
        ...
        let auth = auth_for(ferrum_core::UserId(7), Role::Reseller);
