...
use ferrum_core::{Domain, ErrorCode, FerrumError, Permission, Result, SiteId};
use ferrum_db::{CertKind, DnsProviderKind};
...
impl HttpTransport {
    ...
    pub fn new(token: &SecretToken) -> Result<Self> {
        ...
        let client = reqwest::Client::builder()
            ...
            .build()
            .map_err(|e| FerrumError::internal(format!("could not build an HTTPS client: {e}")))?;
        ...
        let result = self
            .call(CfRequest {
                ...
                body: Some(serde_json::json!({
                    ...
                    "content": content,
                    "ttl": 60,
                    "comment": "ferrum ACME DNS-01 challenge (temporary)",
                })),
            })
            .await?;
...
async fn server_public_addresses(ctx: &OpContext) -> Vec<IpAddr> {
    let configured: Vec<String> = ctx
        .db()
        .get_setting_or(ferrum_db::settings::keys::DNS_SERVER_ADDRESSES, Vec::new())
        .await;
...
fn validate_label(label: &str) -> Result<String> {
    ...
    if trimmed.is_empty() {
        return Err(
            FerrumError::new(ErrorCode::InvalidInput, "the credential needs a label")
                .with_field("label"),
        );
    }
            ...
            ctx.master_key()
                ...
                .map_err(|e| {
                    FerrumError::internal(format!(
                     ...
                     If /etc/ferrum/secret.key was replaced, set the token again.",
                        provider.label
                    ))
                })?,
        );
    ...
    Fut: Future<Output = Result<T>>,
{
    ...
    let mut outcome: Result<T> = Err(FerrumError::internal("the challenge body never ran"));
...
mod tests {
    ...
    async fn the_challenge_records_are_removed_when_the_order_fails() {
        ...
        let err = with_challenge_records(&cf, "z1", &records, &|_| {}, || async {
            Err::<(), _>(FerrumError::new(ErrorCode::CommandFailed, "the CA said no"))
        })
        .await
        .unwrap_err();
    ...
    async fn a_cleanup_failure_does_not_mask_the_reason_the_order_failed() {
        ...
        let err = with_challenge_records(&cf, "z1", &records, &|_| {}, || async {
            Err::<(), _>(FerrumError::new(ErrorCode::RateLimited, "too many orders"))
        })
        .await
        .unwrap_err();
    ...
    async fn a_reseller_cannot_replace_the_servers_dns_credential() {
        ...
        let reseller = reg
            ...
            .create(ferrum_db::users::NewUser {
                role: Role::Reseller,
                email: ferrum_core::Email::parse("reseller@example.com").unwrap(),
                username: ferrum_core::Username::parse("reseller").unwrap(),
