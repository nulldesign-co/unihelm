...
use ferrum_core::{Domain, LinuxUser, Result};
...
pub async fn ensure_site_dirs(
    ...
    let root = ferrum_config::paths::site_root(user.as_str(), domain.as_str());
    ...
    let log_dir = ferrum_config::paths::site_log_dir(domain.as_str());
    Cmd::new("mkdir")
        .args(["-p", "--"])
        .arg(&log_dir)
        .run_checked()
...
pub async fn write_placeholder(user: &LinuxUser, domain: &Domain) -> Result<()> {
    let path = ferrum_config::paths::site_public(user.as_str(), domain.as_str()).join("index.html");
    if path.exists() {
        return Ok(());
    }
    ...
    std::fs::write(&path, body).map_err(|e| {
        ferrum_core::FerrumError::internal(format!("could not write {}: {e}", path.display()))
    })?;
...
pub async fn remove_site_dirs(user: &LinuxUser, domain: &Domain) -> Result<()> {
    let root = ferrum_config::paths::site_root(user.as_str(), domain.as_str());
    let expected_prefix = ferrum_config::paths::tenant_home(user.as_str()).join("sites");

    if !root.starts_with(&expected_prefix) || root.components().count() < 5 {
        return Err(ferrum_core::FerrumError::internal(format!(
...
mod tests {
    ...
    fn the_site_layout_matches_the_documented_one() {
        let root = ferrum_config::paths::site_root("ft_abc12345", "example.com");
        assert_eq!(
            root.to_str().unwrap(),
            "/home/ft_abc12345/sites/example.com"
        );
    ...
    async fn removal_refuses_a_path_outside_the_tenant_home() {
        ...
        let root = ferrum_config::paths::site_root(user().as_str(), domain().as_str());
        let expected = ferrum_config::paths::tenant_home(user().as_str()).join("sites");
    ...
    fn logs_live_outside_the_tenant_home() {
        ...
        let log_dir = ferrum_config::paths::site_log_dir("example.com");
