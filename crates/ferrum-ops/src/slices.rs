...
//!
//! Every customer subscription gets one slice unit, `ferrum-<user>.slice`,
...
//!
//! In slice unit names every `-` is a nesting level, so `ferrum-ft_ab.slice`
//! automatically sits under an implicit `ferrum.slice` that systemd creates on
...
use ferrum_core::{ErrorCode, FerrumError, LinuxUser, Result};
use ferrum_distro::svc::{SvcAction, UnitName};
...
const SLICE_DROPIN: &str = "ferrum-slice.conf";
...
/// The tenant's slice unit file name: `ferrum-<user>.slice`.
///
/// Hyphens in the user are escaped to `\x2d` (systemd's own escaping) because
/// in *slice* names a `-` is a nesting separator: `ferrum-a-b.slice` would be
/// a grandchild slice `ferrum.slice/ferrum-a.slice/ferrum-a-b.slice`, and two
...
/// `ferrum.slice`, siblings, whatever their names.
pub fn slice_file_name(user: &LinuxUser) -> String {
    format!("ferrum-{}.slice", user.as_str().replace('-', "\\x2d"))
}
...
/// Where the slice unit lives: `/etc/systemd/system/ferrum-<user>.slice`.
...
pub async fn remove_tenant_slice_at(
    ...
    let status = distro.svc.status(&unit).await.map_err(FerrumError::from)?;
...
/// this module owns. Only `ferrum-*.service` units are accepted: a drop-in on
...
pub async fn apply_unit_slice_dropin_at(
    ...
    if !unit.as_str().starts_with("ferrum-") {
        return Err(FerrumError::new(
            ErrorCode::InvalidInput,
            format!(
                "refusing to move `{unit}` into a tenant slice: only the panel's own \
                 per-tenant units (ferrum-*) belong to a single tenant"
            ),
        ));
    }
...
mod tests {
    ...
    use ferrum_core::{AuthContext, Role, TenantScope, UserId};
    ...
    fn an_unlimited_slice_renders_accounting_and_nothing_else() {
        ...
        assert_eq!(
            body,
            "[Unit]\n\
             Description=Ferrum tenant ft_abc12345 resource limits\n\
             \n\
             [Slice]\n\
    ...
    fn the_dropin_lands_a_unit_in_the_tenant_slice() {
        let body = TemplateSet::load()
            ...
            .unwrap();
        assert_eq!(body, "[Service]\nSlice=ferrum-ft_abc12345.slice\n");
    }
    ...
    fn slice_names_follow_the_tenant_user() {
        assert_eq!(slice_file_name(&user()), "ferrum-ft_abc12345.slice");
        assert_eq!(
            slice_unit_name(&user()).as_str(),
            "ferrum-ft_abc12345.slice"
        );
    }
    ...
    fn a_hyphenated_user_is_escaped_not_nested() {
        ...
        assert_eq!(slice_file_name(&hyphenated), "ferrum-ab\\x2dcd.slice");
    ...
    fn every_valid_linux_user_yields_a_valid_unit_name() {
        ...
        for name in ["a", "_x", "ft_abc12345", "a-b-c-d", "z_9-", &"a".repeat(32)] {
    ...
    fn fpm_pool_sizing_follows_the_tenant_memory_budget() {
        ...
        let pool = ferrum_config::PoolContext::new(
            "example.com",
            "ft_abc12345",
            ferrum_core::PhpVersion::V83,
    ...
    async fn a_foreign_unit_file_is_never_overwritten() {
        // An operator's hand-written ferrum-<user>.slice is *their* statement
    ...
    async fn removal_is_idempotent() {
        ...
        let never = dir.path().join("ferrum-ft_never.slice");
    ...
    async fn a_service_dropin_lands_the_unit_in_the_tenant_slice() {
        let (ctx, _rec) = ctx(Family::Debian).await;
        let dir = tempfile::tempdir().unwrap();
        let unit = UnitName::parse("ferrum-app-blog-1.service").unwrap();
        let path = dir
            .path()
    ...
    async fn a_dropin_is_refused_for_units_the_panel_does_not_own() {
        ...
        let slice = UnitName::parse("ferrum-x.slice").unwrap();
        assert!(
