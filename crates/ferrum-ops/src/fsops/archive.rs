...
mod tests {
    ...
    fn a_tar_symlink_is_skipped_and_a_file_through_it_cannot_reach_the_target() {
        ...
        // `link/ferrum-owned`. If the link were created, entry 2 would write
        ...
        through.as_gnu_mut().unwrap().name[..17].copy_from_slice(b"link/ferrum-owned");
        through.set_cksum();
        ...
        assert_eq!(
            std::fs::read(link_path.join("ferrum-owned")).unwrap(),
            content
        );
        assert!(!Path::new("/etc/ferrum-owned").exists());
    }
