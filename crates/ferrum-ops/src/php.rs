...
use ferrum_core::{ErrorCode, FerrumError, PhpVersion, Result};
use ferrum_distro::{Family, PackageName};
...
pub fn fpm_package(family: Family, version: PhpVersion) -> Result<PackageName> {
    ...
    PackageName::parse(&name)
        .map_err(|e| FerrumError::new(ErrorCode::InvalidInput, format!("{name}: {e}")))
}
