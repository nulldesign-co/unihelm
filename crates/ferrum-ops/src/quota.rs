...
/// `/home/ft_x`, and `/homework` never matches `/home` (component comparison,
/// not string prefix).
...
/// The real thing: [`ferrum_distro::Cmd`], argv arrays, trusted-dir binary
...
impl QuotaRunner for SystemRunner {
    async fn run(&self, program: &str, args: &[String]) -> Result<RunOutput> {
        ...
        let out = ferrum_distro::Cmd::new(program)
            .args(args)
...
impl QuotaBackend for DuFallback {
    ...
    async fn usage(&self, target: &QuotaTarget) -> Result<QuotaUsage> {
        ...
        let bytes = tokio::task::spawn_blocking(move || walk_usage_bytes(&dir))
            .await
            .map_err(|e| FerrumError::internal(format!("usage walk panicked: {e}")))?;
        Ok(QuotaUsage {
...
impl TypedOperation for Usage {
    ...
    async fn run(&self, ctx: &OpContext, input: Self::Input) -> Result<Self::Output> {
        ...
        let limits = ctx
            ...
            .await
            .map_err(FerrumError::from)?
            .unwrap_or(ferrum_db::QuotaLimits {
                quota_soft_mb: None,
                quota_hard_mb: None,
            });
...
/dev/sda1 / ext4 rw,relatime 0 0";
    ...
    fn the_longest_matching_mount_point_wins() {
        ...
        // shadow it.
        let entries = parse_mounts(XFS_PRJQUOTA);
        let entry = mount_for(&entries, Path::new("/home/ft_abc/sites")).unwrap();
    ...
    fn setquota_argv_snapshot_converts_mb_to_kib_blocks() {
        ...
        assert_eq!(
            args,
            vec!["-u", "ft_a1b2c3d4", "512000", "563200", "0", "0", "/home"]
        );
    }
    ...
    fn user_quota_usage_argv_snapshot() {
        ...
        assert_eq!(args, vec!["-w", "-u", "ft_a1b2c3d4"]);
    }
    ...
    fn target(project: Option<i64>) -> QuotaTarget {
        QuotaTarget {
            linux_user: user(),
            home_dir: PathBuf::from("/home/ft_a1b2c3d4"),
            project_id: project,
        }
    }
    ...
    async fn the_xfs_backend_tags_the_tree_before_limiting_it() {
        ...
        assert_eq!(calls[0].1[2], "project -s -p /home/ft_a1b2c3d4 101");
    ...
    async fn ext4_usage_reads_an_over_quota_report_despite_the_nonzero_exit() {
        ...
        let rec = RecordingRunner::new(vec![RunOutput {
            status: 1,
            stdout: "Disk quotas for user ft_a1b2c3d4 (uid 1001):\n\
    ...
    async fn ext4_usage_for_a_user_with_no_quota_record_is_zero() {
        let rec = RecordingRunner::new(vec![RunOutput {
            status: 0,
            stdout: "Disk quotas for user ft_a1b2c3d4 (uid 1001): none\n".into(),
            stderr: String::new(),
        }]);
    ...
    async fn a_missing_home_counts_as_empty_not_as_an_error() {
        ...
        let mut t = target(None);
        t.home_dir = PathBuf::from("/nonexistent/ferrum-test-home");
        let usage = DuFallback.usage(&t).await.unwrap();
        assert_eq!(usage.used_mb, 0);
    }
