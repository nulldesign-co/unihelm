...
//!
//! `ferrum-agentd --wp-helper` re-execs the agent binary, drops to the tenant's
...
use ferrum_core::{ErrorCode, FerrumError, Result};
...
pub async fn run_wp_cli(
    ...
    let phar = ferrum_config::paths::wp_cli_phar();
    if !phar.exists() {
    ...
    // Resolved against `ferrum_distro`'s fixed list of trusted directories, so
    ...
    let php = ferrum_distro::exec::resolve_program("php").map_err(|_| {
        FerrumError::new(
            ErrorCode::NotFound,
    ...
    let out = Cmd::new(php.to_string_lossy().into_owned())
        ...
        // Ferrum's task log is the progress display; WP-CLI's ANSI colouring
...
fn truncate(mut text: String) -> String {
    ...
    text.truncate(cut);
    text.push_str("\n… [truncated by Ferrum: WP-CLI produced more than 1 MiB]\n");
    text
}
...
mod tests {
    ...
    fn dir() -> PathBuf {
        PathBuf::from("/home/ft_x/sites/example.com/public")
    }
    ...
    fn oversized_output_is_cut_and_says_so() {
        ...
        assert!(cut.contains("truncated by Ferrum"));
