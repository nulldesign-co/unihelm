...
//!
//! **On a stock Ferrum server this feature refuses to enable, and it says so.**
//! Ferrum installs nginx from nginx.org (`ferrum_distro::repos::nginx`), and
//! nginx.org publishes no ModSecurity module. Checked on 2026-08-28 against the
//! four package trees Ferrum can install from:
//!
...
//! panel does not do (spec §10.4 rule 1, `ferrum_config::paths`).
//!
...
//! to load it from — an operator's own build, a distro nginx, the future Ferrum
...
//! Ferrum does not do that, for two reasons: it would mean re-rendering every
...
//! `ferrum.d/03-waf.conf`, starting in `DetectionOnly`, and each site's policy
...
use ferrum_core::{ErrorCode, FerrumError, Permission, Result, SiteId, TenantScope};
use ferrum_db::settings::keys;
use ferrum_db::{Db, NewWafExclusion, WafExclusion, WafMode};
use ferrum_distro::{Cmd, Family};
...
/// The OWASP Core Rule Set release Ferrum installs.
...
/// `ferrum_distro::repos::UNVERIFIED_PINS`: it detects a later tampered or
...
/// OpenPGP signature beside every asset (`…-minimal.tar.gz.asc`). Ferrum's
/// in-tree OpenPGP code (`ferrum_distro::pgp`) parses keys and computes
...
/// The base of Ferrum's per-site rule ids.
///
/// The Core Rule Set reserves 900,000–999,999 for itself and documents
/// 1–99,999 as the range for local rules. 20,000 keeps Ferrum clear of the
...
/// One past the last id Ferrum will allocate.
...
const LOAD_MODULE_DROPIN: &str = "50-ferrum-modsecurity.conf";

// ---------------------------------------------------------------------------
// Preflight: is there a WAF to configure at all?
// ---------------------------------------------------------------------------

/// A packaged ModSecurity connector Ferrum knows about, and the truth about it.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize)]
pub struct ModuleCandidate {
    pub package: &'static str,
    pub repository: &'static str,
    /// The nginx these packages are built against. Recorded because it is the
    /// whole reason installing one does not help on a Ferrum server.
        ...
        blockers.push(Blocker {
            ...
            remedy: format!(
                "There is a packaged connector — {} — but an nginx dynamic module \
                 ...
                 module built for this nginx: spec §11.9 plans one in Ferrum's own \
        ...
        blockers.push(Blocker {
            ...
            remedy: format!(
                "Add one line to {} — `include /etc/nginx/modules-enabled/*.conf;` \
                 above the `events` block — and create that directory. Ferrum will \
...
impl Default for WafSettings {
    fn default() -> Self {
        Self {
            ...
            default_paranoia: ferrum_db::waf::DEFAULT_PARANOIA,
        }
    }
}
...
pub fn validate_paranoia(level: i64) -> Result<()> {
    if (ferrum_db::waf::MIN_PARANOIA..=ferrum_db::waf::MAX_PARANOIA).contains(&level) {
        return Ok(());
    }
    Err(FerrumError::new(
        ErrorCode::InvalidInput,
        format!(
            "paranoia level must be between {} and {}; CRS defines no other level, \
             ...
             showed {level}",
            ferrum_db::waf::MIN_PARANOIA,
            ferrum_db::waf::MAX_PARANOIA,
            ferrum_db::waf::MIN_PARANOIA
        ),
    )
...
pub fn extract_crs(tar_gz: &[u8], dest: &Path) -> Result<usize> {
    ...
    let entries = archive.entries().map_err(|e| {
        FerrumError::internal(format!("the Core Rule Set archive is unreadable: {e}"))
    })?;
    ...
    std::fs::create_dir_all(dest)
        .map_err(|e| FerrumError::internal(format!("could not create {}: {e}", dest.display())))?;
    ...
    for (index, entry) in entries.enumerate() {
        ...
        if kind.is_dir() {
            std::fs::create_dir_all(&target).map_err(|e| {
                FerrumError::internal(format!("could not create {}: {e}", target.display()))
            })?;
            continue;
        }
        if let Some(parent) = target.parent() {
            std::fs::create_dir_all(parent).map_err(|e| {
                FerrumError::internal(format!("could not create {}: {e}", parent.display()))
            })?;
        }
        ...
        std::fs::write(&target, &buf).map_err(|e| {
            FerrumError::internal(format!("could not write {}: {e}", target.display()))
        })?;
        written += 1;
    }
    Ok(written)
        ...
        return Err(FerrumError::new(
            ...
            format!(
                "the unpacked Core Rule Set has no {} — the archive layout is not \
                 what this version of Ferrum expects",
                example.display()
            ),
        ));
    }
    std::fs::copy(&example, &setup)
        .map_err(|e| FerrumError::internal(format!("could not write {}: {e}", setup.display())))?;
...
async fn site_views(db: &Db) -> Result<Vec<SitePolicyView>> {
    let policies = db.waf_site_policies().await.map_err(FerrumError::from)?;
    if policies.is_empty() {
        return Ok(Vec::new());
    }

    let exclusions = db.waf_exclusions().await.map_err(FerrumError::from)?;
...
fn ensure_dir(path: &Path, mode: u32) -> Result<()> {
    ...
    std::fs::create_dir_all(path)
        .map_err(|e| FerrumError::internal(format!("could not create {}: {e}", path.display())))?;
    std::fs::set_permissions(path, std::fs::Permissions::from_mode(mode)).map_err(|e| {
        FerrumError::internal(format!("could not set mode on {}: {e}", path.display()))
    })?;
    Ok(())
}
...
fn refusal(blockers: &[Blocker]) -> FerrumError {
    let detail = blockers
        .iter()
...
pub struct StatusOutput {
    ...
    /// `blockers` is the honest answer on a stock Ferrum server today.
    pub available: bool,
    pub blockers: Vec<Blocker>,
    pub module: ModuleState,
    pub load: LoadPlan,
    ...
    async fn run(&self, ctx: &OpContext, input: Self::Input) -> Result<Self::Output> {
        ...
        match input.site_id {
            Some(site_id) => {
                ...
                db.set_waf_site_policy(site_id, WafMode::Off, ferrum_db::waf::DEFAULT_PARANOIA)
                    .await
                    .map_err(FerrumError::from)?;
...
async fn require_site(db: &Db, site_id: i64) -> Result<String> {
    db.sites(&TenantScope::Global)
        ...
        .map(|s| s.domain)
        .ok_or_else(|| {
            FerrumError::new(ErrorCode::NotFound, format!("no site with id {site_id}"))
                .with_field("site_id")
        })
}
...
";
    ...
    fn nginx_orgs_own_config_offers_nowhere_to_load_a_module_from() {
        ...
        // Ferrum server, and it is a property of the file nginx.org ships.
    ...
    fn a_distro_nginx_with_a_modules_enabled_directory_gets_a_dropin() {
        ...
        match plan_module_load(DEBIAN_NGINX_CONF, &[]) {
            LoadPlan::Dropin { path } => {
                assert_eq!(
                    path,
                    "/etc/nginx/modules-enabled/50-ferrum-modsecurity.conf"
                )
            }
    ...
    fn a_present_module_with_somewhere_to_load_it_is_available() {
        let pre = assess(
            ...
            LoadPlan::Dropin {
                path: "/etc/nginx/modules-enabled/50-ferrum-modsecurity.conf".into(),
            },
        );
        assert!(
            pre.is_available(),
    ...
    fn rule_ids_stay_inside_the_block_the_core_rule_set_leaves_for_local_rules() {
        ...
        // CRS owns 900000-999999; anything Ferrum emits must stay well clear.
    ...
    fn the_nginx_include_points_at_the_rules_file_and_nothing_else() {
        ...
        let rendered = templates
            .render(
                "nginx/waf.conf",
                &serde_json::json!({ "rules_file": "/etc/ferrum/waf/main.conf" }),
            )
            .unwrap();
        assert!(rendered.contains("modsecurity on;"));
        assert!(rendered.contains("modsecurity_rules_file /etc/ferrum/waf/main.conf;"));
    ...
    fn an_absolute_archive_entry_is_refused() {
        let dir = tempfile::tempdir().unwrap();
        let evil = hostile_tar_gz("/etc/ferrum/secret.key", b"owned");
        let err = extract_crs(&evil, dir.path()).unwrap_err();
        assert!(err.detail.contains("absolute path"), "{}", err.detail);
        assert!(!Path::new("/etc/ferrum/secret.key.test").exists());
    }
    ...
    fn a_symlink_entry_is_refused_rather_than_followed() {
        ...
        builder
            .append_link(&mut header, "escape", "/etc/ferrum")
            .unwrap();
        let tar = builder.into_inner().unwrap();
        let mut encoder = flate2::write::GzEncoder::new(Vec::new(), flate2::Compression::default());
        std::io::Write::write_all(&mut encoder, &tar).unwrap();
