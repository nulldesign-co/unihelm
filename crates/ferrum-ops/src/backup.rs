...
//! `ferrum_distro::exec`), so the agent's own environment never leaks into
...
//! panel seals a copy under [`ferrum_db::MasterKey`] so the scheduler can run
...
//! 2. `/etc/ferrum/secret.key`, the master key — because every other secret in
...
//! `/etc/ferrum` and the state directory, which is the whole of the panel's
...
use ferrum_core::{ErrorCode, FerrumError, Permission, Result, SubscriptionId, TenantScope};
use ferrum_db::backups::{BackupScope, NewBackupRepo, RepoKind, RunOutcome};
use ferrum_distro::pkg::PackageName;
use ferrum_distro::{Cmd, exec};
...
const PANEL_TAG: &str = "ferrum-panel";
...
/// up at 30 s (`ferrum_ipc::client::DEFAULT_CALL_TIMEOUT`) — better to fail
...
impl ResticInvocation {
    ...
    async fn run(&self) -> Result<ferrum_distro::CmdOutput> {
        self.to_cmd().run().await.map_err(FerrumError::from)
    }

    /// Run, streaming every line into the task log through `sink`.
    async fn run_streaming<F>(&self, sink: F) -> Result<ferrum_distro::CmdOutput>
    where
        F: FnMut(&str) + Send,
    {
        self.to_cmd()
...
pub fn scope_tag(scope: BackupScope, subscription_id: Option<SubscriptionId>) -> String {
    match (scope, subscription_id) {
        ...
        (BackupScope::Subscription, Some(id)) => format!("ferrum-sub-{}", id.get()),
        ...
        (BackupScope::Subscription, None) => "ferrum-sub-unknown".to_string(),
    }
}
...
///
/// **Duplication, knowingly.** `crates/ferrum-ops/src/cron.rs` is being written
...
fn invalid(detail: impl Into<String>) -> FerrumError {
    FerrumError::new(ErrorCode::InvalidInput, detail)
}
...
async fn ensure_restic(ctx: &OpContext, program: &str) -> Result<()> {
    ...
    let package = PackageName::parse(RESTIC_PACKAGE).map_err(FerrumError::from)?;
    let installed = ctx
        .distro()
        .pkg
...
async fn resolve_repo(ctx: &OpContext, repo_id: i64) -> Result<RepoTarget> {
    let repo = ctx
        ...
        .await
        .map_err(FerrumError::from)?
        .ok_or_else(|| FerrumError::not_found("backup repository"))?;
    ...
    let credentials = match secrets.credentials_sealed {
        Some(sealed) => {
            ...
            Some(serde_json::from_str::<S3Credentials>(&json).map_err(|e| {
                FerrumError::internal(format!("stored credentials are not readable: {e}"))
            })?)
        }
        None => None,
    };
...
///
/// A development instance has no `/etc/ferrum`, and a panel that has never
...
pub const PASSWORD_NOTICE: &str = "This password is shown once and cannot be recovered from the \
     panel. Store it off this server, together with /etc/ferrum/secret.key: without both, a \
...
impl TypedOperation for RepoInit {
    ...
    async fn run(&self, ctx: &OpContext, input: Self::Input) -> Result<Self::Output> {
        ...
        let password_sealed = ctx
            .master_key()
            .seal_str(&password)
            .map_err(|e| FerrumError::internal(format!("could not seal the password: {e}")))?;
        let credentials_sealed = match &credentials {
            Some(c) => {
                let json = serde_json::to_string(c)
                    .map_err(|e| FerrumError::internal(format!("credentials: {e}")))?;
                Some(ctx.master_key().seal_str(&json).map_err(|e| {
                    FerrumError::internal(format!("could not seal the credentials: {e}"))
                })?)
            }
            None => None,
        };
    ...
    async fn resolve_scope(
        ...
        match input.scope {
            BackupScope::Panel => {
                ...
                // The panel scope carries `panel.db`, `/etc/ferrum` and every
...
async fn authorise_repository(
    ...
    if allowed {
        ...
        // at all.
        Err(FerrumError::not_found("backup repository"))
    }
}
...
pub async fn run_backup(
    ...
    std::fs::create_dir_all(&work_dir).map_err(|e| {
        FerrumError::internal(format!("could not create {}: {e}", work_dir.display()))
    })?;
    ...
    let paths = match scope {
        ...
        BackupScope::Subscription => {
            let id = subscription.ok_or_else(|| {
                FerrumError::internal("a subscription backup reached the runner without a tenant")
            })?;
            let sub = ctx
                .db()
...
impl TypedOperation for Restore {
    ...
    async fn run(&self, ctx: &OpContext, input: Self::Input) -> Result<Self::Output> {
        ...
        let stamp = ferrum_db::to_sql_time(ferrum_db::now()).replace(':', "-");
        let staging = self
            .state_root()
            .join("restore")
            .join(format!("{stamp}-{snapshot_id}"));
        std::fs::create_dir_all(&staging).map_err(|e| {
            FerrumError::internal(format!("could not create {}: {e}", staging.display()))
        })?;
        // A restored tree can contain `/etc/ferrum/secret.key` and every
        ...
        Ok(RestoreOutput {
            ...
            next_steps: vec![
                ...
                "Copy back what you need, then delete the staging directory — it may \
                 contain /etc/ferrum/secret.key and tenant data."
                    .into(),
            ],
        })
    }
...
impl TypedOperation for ScheduleSet {
    ...
    type Output = ferrum_db::backups::BackupSchedule;
    ...
    async fn run(&self, ctx: &OpContext, input: Self::Input) -> Result<Self::Output> {
        ...
        if ctx
            ...
            .is_none()
        {
            return Err(FerrumError::not_found("backup repository"));
        }
        ...
        ctx.db()
            .create_backup_schedule(ferrum_db::backups::NewBackupSchedule {
                repo_id: input.repo_id,
                scope: input.scope,
...
///
/// Called from `ferrum-agentd`'s scheduler under the system identity. Like the
...
mod tests {
    ...
    use ferrum_db::backups::{NewBackupSchedule, RunStatus};
    use ferrum_distro::mock::{RecordingLog, SharedRecorder};
    use std::sync::Arc;
    ...
    fn target() -> RepoTarget {
        RepoTarget {
            ...
            label: "nightly".into(),
            kind: RepoKind::S3,
            location: "s3.example.com/ferrum".into(),
    ...
    fn the_repository_password_reaches_restic_through_the_environment_and_never_argv() {
        ...
        for args in [
            init_args(),
            backup_args("ferrum-panel", &[PathBuf::from("/var/lib/ferrum")]),
            snapshots_args(Some("ferrum-sub-3")),
            restore_args("abc12345", Path::new("/var/lib/ferrum/state/restore/x")),
            forget_args(
                "ferrum-panel",
                KeepPolicy {
                    daily: 7,
            ...
            assert_eq!(env["RESTIC_REPOSITORY"], "s3:s3.example.com/ferrum");
        }
    }
    ...
    fn the_backup_argv_separates_flags_from_paths() {
        let args = backup_args("ferrum-panel", &[PathBuf::from("/etc/ferrum")]);
        let dash_dash = args.iter().position(|a| a == "--").expect("`--` present");
        assert_eq!(args[dash_dash + 1], "/etc/ferrum");
    ...
    fn retention_argv_is_scoped_to_one_tag_and_actually_prunes() {
        ...
        assert_eq!(args[pos("--tag") + 1], "ferrum-sub-42");
    ...
    fn a_subscription_tag_names_the_id_not_the_linux_user() {
        ...
        assert_eq!(scope_tag(BackupScope::Panel, None), "ferrum-panel");
    }
    ...
    const SNAPSHOTS_FIXTURE: &str = r#"[
      {
        ...
        "paths": ["/var/lib/ferrum/state", "/etc/ferrum"],
        "hostname": "web-01",
        "username": "root",
        "tags": ["ferrum-panel"],
    ...
    fn the_snapshot_list_survives_fields_the_panel_has_never_seen() {
        ...
        assert_eq!(snapshots[0].tags, vec!["ferrum-panel"]);
        assert_eq!(snapshots[1].paths, vec!["/home/ft_ab12cd"]);
    ...
    fn an_s3_location_needs_a_bucket_and_keeps_one_scheme_prefix() {
        ...
        assert_eq!(
            check_s3_url("s3:s3.example.com/ferrum").unwrap(),
            "s3.example.com/ferrum",
            "a pasted `s3:` must not become `s3:s3:`"
        );
        assert_eq!(
            check_s3_url("https://minio.example.com/ferrum/panel").unwrap(),
            "https://minio.example.com/ferrum/panel"
        );
    }
    ...
    async fn file_backed_context(
        ...
        use ferrum_core::{Email, Username};
        use ferrum_db::users::NewUser;

        let db = ferrum_db::Db::open(db_dir.join("panel.db")).await.unwrap();
        ...
        let err = RepoInit::with_program("echo")
            .run(
                ...
                RepoInitInput {
                    kind: RepoKind::S3,
                    label: "offsite".into(),
                    path_or_url: "s3.example.com/ferrum".into(),
                    s3: None,
                },
            )
            .await
    ...
    async fn a_panel_backup_never_writes_the_repository_password_to_the_task_log() {
        ...
        assert!(log.contains("backup --json --tag ferrum-panel"), "{log}");
    ...
    async fn a_scheduled_run_prunes_under_that_schedules_policy() {
        ...
        let log = log_text(&rec);
        assert!(log.contains("forget --prune --tag ferrum-panel"), "{log}");
