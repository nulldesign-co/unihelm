...
//!
//! Ferrum v1 runs **no mail server**. It stores the address of somebody else's
...
//! - the file is under `/etc/ferrum/mail`, not in the tenant's home, so a
...
use ferrum_config::{CommentStyle, ManagedFile, paths};
use ferrum_core::{ErrorCode, FerrumError, LinuxUser, Permission, Result, TenantScope};
use ferrum_db::{MailRelay, NewMailRelay, TlsMode};
...
pub fn sendmail_agent_path() -> Option<PathBuf> {
    ferrum_distro::exec::resolve_program(SENDMAIL_AGENT).ok()
}
...
/// [`ferrum_distro::exec::resolve_program`] found in a fixed list of trusted
...
pub async fn write_site_relay(
    ...
    std::fs::create_dir_all(paths::mail_dir()).map_err(|e| {
        FerrumError::internal(format!("could not create the mail config directory: {e}"))
    })?;
...
fn remove_site_relay(path: &std::path::Path) -> Result<()> {
    match std::fs::remove_file(path) {
        ...
        Err(e) => Err(FerrumError::internal(format!(
            "could not remove {}: {e}",
            path.display()
        ))),
    }
...
pub async fn rewire_all_sites(
    ...
    let mut first_error: Option<FerrumError> = None;
...
pub fn dns_advisory(relay: Option<&MailRelay>) -> DnsAdvisory {
    ...
    let records = vec![
        ...
        AdvisoryRecord {
            ...
            value: None,
            managed: false,
            purpose: "DKIM: the relay signs the mail and publishes the public half here. Ferrum \
    ...
    let advice = if recognised {
        format!(
            "These records are for `{domain}`, the domain the relay sends as. The SPF mechanism \
             ...
             Ferrum neither signs nor manages any of these.",
            relay.host
        )
    } else {
        format!(
            "These records are for `{domain}`, the domain the relay sends as. `{}` is not a \
             relay Ferrum recognises, so the SPF mechanism below points at that host directly — \
             correct for a single-host relay, and incomplete for a provider with a fleet. Use \
             the mechanism the provider publishes if it has one. Ferrum neither signs nor \
...
impl TypedOperation for RelaySet {
    ...
    async fn run(&self, ctx: &OpContext, input: Self::Input) -> Result<Self::Output> {
        ...
        let existing = ctx.db().mail_relay().await.map_err(FerrumError::from)?;
        let password_sealed = match input.password.as_deref() {
            ...
            Some(secret) => {
                ...
                if let Err(detail) = smtp::reject_control_characters("password", secret) {
                    return Err(
                        FerrumError::new(ErrorCode::InvalidInput, detail).with_field("password")
                    );
                }
                Some(ctx.master_key().seal_str(secret).map_err(|e| {
                    FerrumError::internal(format!("the relay password could not be sealed: {e}"))
                })?)
            }
        };
