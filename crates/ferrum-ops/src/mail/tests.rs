...
async fn seed_php_site(db: &Db, customer: ferrum_core::UserId, domain: &str, php: bool) {
    ...
    let site = db
        ...
        .await
        .unwrap();
    db.set_site_status(site.id, ferrum_db::SiteStatus::Active)
        .await
        .unwrap();
}
...
fn an_unknown_relay_gets_an_honest_fallback_and_is_told_it_is_a_fallback() {
    ...
    assert!(advisory.advice.contains("not a relay Ferrum recognises"));
}
...
fn the_relay_config_path_is_under_etc_and_not_in_the_tenant_home() {
    ...
    let path = ferrum_config::paths::mail_site_config("example.com");
    assert!(path.starts_with("/etc/ferrum/mail"), "{path:?}");
