...
/// `ferrum_ipc::DEFAULT_CALL_TIMEOUT` (30 s) on purpose: a relay that accepts
...
/// default. `ferrum-ops` already links rustls with `aws-lc-rs` for ACME, and
...
///
/// Plain ASCII passes through unchanged — an encoded-word around `Ferrum test
...
mod tests {
    ...
    fn message() -> Message {
        Message {
            ...
            from_name: Some("Ferrum".into()),
            to: "ops@example.com".into(),
            subject: "Ferrum relay test".into(),
            body: "This is a test.\n".into(),
        }
    }
    ...
    fn every_header_line_ends_with_crlf_and_the_body_is_separated_by_a_blank_line() {
        ...
        assert!(rendered.starts_with("From: Ferrum <panel@example.com>\r\n"));
        assert!(rendered.contains("\r\n\r\nThis is a test.\r\n"));
    ...
    fn a_non_ascii_display_name_is_rfc_2047_encoded_and_ascii_is_left_alone() {
        ...
        assert_eq!(encode_header_word("Ferrum"), "Ferrum");
