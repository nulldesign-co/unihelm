...
//!
//! `ferrum-agentd --pty-helper` re-execs the agent binary, and `main.rs` calls
...
mod tests {
    ...
    fn the_environment_is_built_from_scratch_and_carries_no_inherited_variables() {
        ...
        assert!(text.contains(&"USER=ft_demo".to_string()));
        assert!(text.contains(&"HOME=/home/ft_demo".to_string()));
