...
use ferrum_config::apply::{
    NewRevision, PostCheck, Reloader, RevisionStore, StoredRevision, Validator,
};
use ferrum_config::managed::{CommentStyle, with_header};
use ferrum_config::{ApplyRequest, ConfigEngine, ConfigError, FileState, ManagedFile, TemplateSet};
...
async fn a_validation_failure_restores_the_previous_file_exactly() {
    ...
    let bad = FakeValidator::failing(
        "nginx: [emerg] invalid parameter \"quic\" in /etc/nginx/ferrum.d/site.conf:12",
    );
    let err = engine
