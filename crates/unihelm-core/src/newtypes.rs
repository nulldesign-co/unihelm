...
fn err(code: ErrorCode, detail: impl Into<String>) -> UnihelmError {
...
/// A Linux account name the panel may create or act as (`uh_<short-id>`).
///
...
/// The PHP versions Unihelm knows how to install (spec §11.3).
///
...
/// three:
///
/// - a **systemd unit name** (`unihelm-app-<user>-<name>.service`), where a
...
mod tests {
    ...
    #[test]
    fn linux_user_rules() {
        assert!(LinuxUser::parse("uh_a1b2c3").is_ok());
        for bad in [
            "root",
            "nginx",
            "unihelm",
    ...
    fn app_names_compose_into_valid_unit_names_and_paths() {
        ...
        for name in ["a", "blog", "api-v2", "next_app3", &"z".repeat(32)] {
            ...
            let unit = format!("unihelm-app-uh_abc12345-{app}.service");
            assert!(
                unit.bytes()
