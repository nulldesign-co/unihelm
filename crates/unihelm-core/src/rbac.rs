...
//!    from the session, ships it across the IPC boundary, and `unihelm-agentd`
    ...
    pub fn default_permissions(self) -> &'static [Permission] {
        ...
        match self {
            ...
            Role::Customer => &[
                ...
                // `unihelm_ops::terminal::authorize` re-reads the target
...
///
/// Built once per request in `unihelm-web`, serialised into the IPC envelope, and
/// re-validated by `unihelm-agentd` — never trusted blindly on either side.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
