...
/// The unit file name for an app: `unihelm-app-<user>-<name>.service`.
///
...
pub fn unit_file_name(user: &LinuxUser, name: &AppName) -> String {
    format!("unihelm-app-{}-{}.service", user.as_str(), name.as_str())
}
...
/// `/etc/systemd/system/unihelm-app-<user>-<name>.service`.
...
fn check_entry(entry: &TenantPath) -> Result<()> {
    if entry.as_str().is_empty() {
        return Err(
            UnihelmError::new(ErrorCode::InvalidPath, "the entry point is empty")
                .with_field("entry"),
        );
    }
...
async fn app_and_user(ctx: &OpContext, app_id: i64) -> Result<(NodeApp, LinuxUser, AppName)> {
    ...
    let app = db
        ...
        .await
        .map_err(UnihelmError::from)?
        .ok_or_else(|| UnihelmError::not_found("node app"))?;
    ...
    let subscription = db
        ...
        .await
        .map_err(UnihelmError::from)?
        .ok_or_else(|| UnihelmError::internal("the app's subscription is missing"))?;
...
async fn tenant_home_of(ctx: &OpContext, user: &LinuxUser) -> Result<String> {
    Ok(ctx
        ...
        .await
        .map_err(UnihelmError::from)?
        .ok_or_else(|| UnihelmError::internal("the subscription vanished mid-create"))?
        .home_dir)
}
...
async fn publish_proxy_site(ctx: &OpContext, app: &NodeApp, domain: Domain) -> Result<i64> {
        ...
        .run(
            ...
            crate::site::CreateInput {
                ...
                proxy_port: Some(u16::try_from(app.port).map_err(|_| {
                    UnihelmError::internal("an allocated app port did not fit in a u16")
                })?),
    ...
    ctx.db()
        .set_node_app_site(app.id, Some(unihelm_core::SiteId(created.site_id)))
        .await
        .map_err(UnihelmError::from)?;
    ctx.log(format!(
        "{} now proxies to port {}",
...
mod tests {
    ...
    fn the_unit_runs_as_the_tenant_from_the_app_directory() {
        ...
        assert!(body.contains("User=uh_abc12345\n"), "{body}");
        assert!(body.contains("Group=uh_abc12345\n"), "{body}");
        assert!(
    ...
    fn the_app_directory_is_created_and_handed_over_argv_by_argv() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                vec!["chmod", "0750", "--", "/home/uh_abc12345/apps"],
                ...
                vec!["chmod", "0750", "--", "/home/uh_abc12345/apps/blog"],
            ]
        );
    ...
    fn a_value_with_spaces_is_quoted_as_one_assignment() {
        ...
        // "Unihelm" — a truncation that looks like an application bug.
        let lines = environment_lines(&[EnvVar {
            key: "GREETING".into(),
            value: "Unihelm Panel v1".into(),
        }])
        .unwrap();
        assert_eq!(lines, vec![r#""GREETING=Unihelm Panel v1""#]);
    ...
    fn the_rendered_unit_is_a_snapshot() {
        ...
        assert_eq!(
            ...
            "[Unit]\n\
             ...
             User=uh_abc12345\n\
             Group=uh_abc12345\n\
             WorkingDirectory=/home/uh_abc12345/apps/blog\n\
             ExecStart=/usr/bin/node /home/uh_abc12345/apps/blog/dist/server.js\n\
    ...
    fn unit_names_carry_both_the_tenant_and_the_app() {
        ...
        assert_eq!(
            ...
            "/etc/systemd/system/unihelm-app-uh_abc12345-blog.service"
        );
    }
    ...
    async fn one_tenant_cannot_read_restart_or_delete_anothers_app() {
        ...
        let intruder = db
            ...
            .create(unihelm_db::users::NewUser {
                role: Role::Reseller,
                email: unihelm_core::Email::parse("nosy@example.com").unwrap(),
                username: unihelm_core::Username::parse("nosy").unwrap(),
    ...
    async fn deleting_an_app_disables_its_unit_and_frees_its_port() {
        ...
        let next = db
            ...
            .await
            .unwrap();
        assert_eq!(next.port, unihelm_db::node_apps::APP_PORT_MIN);
    }
    ...
    async fn deleting_an_app_leaves_its_domain_standing_and_names_it() {
        ...
        let site = db
            .create_site(unihelm_db::NewSite {
                ...
                site_type: unihelm_db::SiteType::Proxy,
                php_version: None,
                root_dir: "/home/x/apps/blog".into(),
                proxy_port: Some(unihelm_db::node_apps::APP_PORT_MIN as u16),
                redirect_target: None,
            })
            .await
            .unwrap();
    ...
    async fn listing_reports_what_systemd_thinks_of_each_app() {
        ...
        assert_eq!(
            before["apps"][0]["port"],
            unihelm_db::node_apps::APP_PORT_MIN
        );
