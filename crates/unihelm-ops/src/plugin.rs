...
//! hardening the panel's own `unihelm-web` unit does, speaking the panel's
//! **existing length-prefixed JSON framing** ([`unihelm_ipc::codec`]) over
...
use unihelm_config::apply::{ApplyRequest, Reloader, Validator, managed_for};
use unihelm_config::paths;
use unihelm_core::{ErrorCode, Permission, Result, UnihelmError};
use unihelm_db::{NewPlugin, PluginRecord, PluginSignature};
use unihelm_distro::svc::{UnitName, UnitState};
...
/// systemd unit name component, a Unix account name (`unihelm-plug-<slug>`,
...
/// `unihelm-plug-` (12) + 19 = 31, one under the 32-character account limit
...
impl Manifest {
    ...
    pub fn parse(source: &str) -> Result<(Self, PluginSlug)> {
        ...
        let manifest: Manifest = toml::from_str(source)
            .map_err(|e| UnihelmError::new(ErrorCode::InvalidInput, format!("plugin.toml: {e}")))?;
...
pub fn validate_entry(entry: &str) -> Result<()> {
    ...
    if path.is_absolute() {
        return Err(
            UnihelmError::new(ErrorCode::InvalidPath, "a plugin path must be relative")
                .with_field("entry"),
        );
    }
...
pub mod minisign {
    ...
    pub fn parse_public_key(text: &str) -> Result<PublicKey> {
        ...
        let line = text
            ...
            .ok_or_else(|| {
                UnihelmError::new(ErrorCode::InvalidInput, "that is not a minisign public key")
            })?;
    ...
    fn malformed(detail: &str) -> UnihelmError {
        UnihelmError::new(
            ErrorCode::InvalidInput,
...
fn collect_files(root: &Path, dir: &Path, out: &mut Vec<String>) -> Result<()> {
    ...
    for entry in entries {
        let entry = entry.map_err(|e| UnihelmError::internal(e.to_string()))?;
        ...
        if meta.is_dir() {
            ...
            let relative = path
                .strip_prefix(root)
                .map_err(|_| UnihelmError::internal("a payload path escaped its root"))?;
...
/// One call to a plugin. Deliberately **not** [`unihelm_ipc::RequestFrame`]:
...
/// shared ([`unihelm_ipc::codec`]); the envelope is not.
...
pub async fn call_at(
    ...
    let payload = serde_json::to_vec(&request)
        .map_err(|e| UnihelmError::internal(format!("plugin request: {e}")))?;
    ...
    let work = async {
        ...
        unihelm_ipc::codec::write_frame(&mut stream, &payload)
            .await
            .map_err(UnihelmError::from)?;
        let frame = unihelm_ipc::codec::read_frame(&mut stream, MAX_PLUGIN_FRAME)
            .await
            .map_err(UnihelmError::from)?
            .ok_or_else(|| {
                UnihelmError::new(
        ...
        match response {
            ...
            PluginResponse::Err { message } => Err(UnihelmError::new(
...
fn install_tree(source: &Path, dest: &Path, manifest: &Manifest) -> Result<()> {
    let io = |e: std::io::Error| UnihelmError::internal(format!("installing the plugin: {e}"));
...
impl TypedOperation for List {
    ...
    async fn run(&self, ctx: &OpContext, _input: Self::Input) -> Result<Self::Output> {
        let plugins = ctx.db().list_plugins().await.map_err(UnihelmError::from)?;
...
impl TypedOperation for Install {
    ...
    async fn run(&self, ctx: &OpContext, input: Self::Input) -> Result<Self::Output> {
        ...
        let manifest_text = std::fs::read_to_string(paths::plugin_manifest(&source))
            .map_err(|e| UnihelmError::new(ErrorCode::NotFound, format!("plugin.toml: {e}")))?;
        ...
        let record = ctx
            ...
            .create_plugin(NewPlugin {
                ...
                manifest: serde_json::to_value(&manifest)
                    .map_err(|e| UnihelmError::internal(format!("manifest: {e}")))?,
...
impl TypedOperation for Enable {
    ...
    async fn run(&self, ctx: &OpContext, input: Self::Input) -> Result<Self::Output> {
        ...
        let record = ctx
            ...
            .await
            .map_err(UnihelmError::from)?
            .ok_or_else(|| UnihelmError::not_found("plugin"))?;
    ...
    async fn run(&self, ctx: &OpContext, input: Self::Input) -> Result<Self::Output> {
        ...
        if unit_path.exists() {
            std::fs::remove_file(&unit_path)
                .map_err(|e| UnihelmError::internal(format!("removing the unit: {e}")))?;
        ...
        if dir.starts_with(paths::plugin_root()) && dir.exists() {
            std::fs::remove_dir_all(&dir)
                .map_err(|e| UnihelmError::internal(format!("removing the tree: {e}")))?;
        }
        ctx.db()
...
mod tests {
    ...
    fn a_plugin_cannot_be_installed_from_a_place_the_tenant_controls() {
        assert!(validate_source("/home/uh_ab12cd34/staged").is_err());
        assert!(validate_source("relative/path").is_err());
        assert!(validate_source("/opt/../home/uh_ab/x").is_err());
        assert!(
    ...
    fn a_signature_from_a_trusted_key_verifies_in_both_algorithms() {
        let signer = TestSigner::new(7, *b"unihelm1");
    ...
    fn a_signature_from_an_untrusted_key_is_refused_by_key_id() {
        let ours = TestSigner::new(7, *b"unihelm1");
    ...
    fn a_forged_key_id_does_not_help_an_attacker() {
        let ours = TestSigner::new(7, *b"unihelm1");
        let forger = TestSigner::new(9, *b"unihelm1");
        let keys = vec![minisign::parse_public_key(&ours.public_key()).unwrap()];
        let message = b"slug = \"acme-dns\"\n";
        let err =
    ...
    fn fake_sidecar(
        ...
        tokio::spawn(async move {
            ...
            unihelm_ipc::codec::write_frame(&mut stream, &body)
                .await
                .ok()?;
            Some(request)
        })
    ...
    fn record(slug: &str, extensions: &[&str], enabled: bool) -> PluginRecord {
        PluginRecord {
            ...
            run_user: format!("unihelm-plug-{slug}"),
            signature: PluginSignature::Minisign,
            enabled,
            last_error: None,
            installed_at: unihelm_db::now(),
            updated_at: unihelm_db::now(),
        }
    }
    ...
    async fn the_unit_template_renders_a_hardened_unprivileged_service() {
        ...
        assert!(rendered.contains("User=unihelm-plug-acme-dns"));
