...
use unihelm_core::{ErrorCode, Permission, Result, SiteId, UnihelmError};
use unihelm_db::{CertKind, Certificate};
...
impl TypedOperation for Issue {
    ...
    async fn run(&self, ctx: &OpContext, input: Self::Input) -> Result<Self::Output> {
        ...
        // `unihelm_agentd::scheduler::renew_certificates`).
        crate::webhook::emit(
            ctx,
            "certificate.renewed",
            serde_json::json!({
                ...
                "site_id": site.id.get(),
                "domains": names,
                "not_after": unihelm_db::to_sql_time(issued.not_after),
