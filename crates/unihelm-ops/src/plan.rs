...
use unihelm_db::subscriptions::{Subscription, SubscriptionStatus};
use unihelm_db::{Db, Plan};
...
impl TypedOperation for Assign {
    ...
    async fn run(&self, ctx: &OpContext, input: Self::Input) -> Result<Self::Output> {
        ...
        let plan = db
            ...
            .await
            .map_err(UnihelmError::from)?
            .ok_or_else(|| UnihelmError::not_found("plan"))?;
...
async fn switch_all_vhosts(
    ...
    let mut first_error: Option<(String, UnihelmError)> = None;
    for site in &sites {
...
mod tests {
    ...
    use serde_json::json;
    use std::sync::Mutex;
    use unihelm_core::{Domain, Role, UserId};
    use unihelm_db::users::NewUser;
    use unihelm_db::{NewSite, SiteType};
