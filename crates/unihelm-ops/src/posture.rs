...
//! `Include /etc/ssh/sshd_config.d/*.conf` on both families (it is how Unihelm's
//! own chrooted-SFTP block gets in — `unihelm_config::paths::sshd_dropin`), and
...
/// summarise in one line. (Unihelm's own SFTP drop-in is exactly such a block,
...
fn read_sshd_files() -> Vec<(String, String)> {
    ...
    if let Ok(contents) = std::fs::read_to_string(&main) {
        ...
        // layout and is the conservative direction — a drop-in Unihelm or an
    ...
    // how a `50-unihelm.conf` overrides a stock `sshd_config`.
    dropins.extend(out);
    dropins
}
...
pub fn evaluate(facts: &PostureFacts) -> Vec<Finding> {
    ...
    if facts.firewall.backend == "none" {
        findings.push(Finding {
            ...
            risk: "Every port anything on this server listens on is reachable \
                   ...
                   did not choose to expose. Unihelm cannot ban an address \
            ...
            remedy: "Install firewalld (RHEL family) or ufw (Debian family) and \
                     ...
                     so restart `unihelm-agentd` afterwards or it will go on \
    ...
    match &facts.mysql_listeners {
        ...
        Observed::Known(listeners) if !listeners.is_empty() => {
            ...
            // accounts. `unihelm_ops::harden` now prevents it at install time;
            ...
            findings.push(Finding {
                ...
                risk: "The database accepts connections from outside this \
                    ...
                    .into(),
                remedy: "Unihelm binds MariaDB to 127.0.0.1 at install time; \
                         something has changed that since. Check \
                         /etc/my.cnf.d/60-unihelm.cnf (or the Debian equivalent) \
    ...
    if !facts.sentinel_enabled {
        findings.push(Finding {
            ...
            remedy: "Turn Sentinel on from the firewall page. Add your own \
                     ...
                     address, and note that Unihelm already refuses to ban \
...
";
    ...
    fn settings_inside_a_match_block_do_not_become_the_global_answer() {
        // Unihelm's own SFTP drop-in is a `Match Group` block. Reading its
...
";
    ...
    async fn a_customer_cannot_read_the_servers_security_posture() {
        ...
        use unihelm_core::{ErrorCode, Role};
