...
use unihelm_distro::repos::{MARIADB_SERIES, POSTGRES_MAJOR};
use unihelm_distro::svc::ManagedUnit;
use unihelm_distro::{Cmd, Distro, Family, PackageName};
...
impl StackComponent {
    ...
    fn repo(self, distro: &Distro) -> Result<unihelm_distro::ResolvedRepo> {
        let info = &distro.info;
        let resolved = match self {
            StackComponent::Nginx => unihelm_distro::repos::nginx(info),
            StackComponent::Php { .. } => unihelm_distro::repos::php(info),
            StackComponent::Mariadb => unihelm_distro::repos::mariadb(info, MARIADB_SERIES),
            StackComponent::Postgres => unihelm_distro::repos::pgdg(info),
        };
        resolved.map_err(|e| UnihelmError::new(ErrorCode::UnsupportedDistro, e))
    }
...
mod tests {
    ...
    fn php_pulls_its_family_specific_package_set() {
        ...
        let (rhel, _) =
            unihelm_distro::mock::mock_distro_with_recorder(unihelm_distro::Family::Rhel);
        let packages = StackComponent::Php {
            version: PhpVersion::V83,
        }
    ...
    fn every_component_resolves_a_pinned_repository_on_both_families() {
        for family in [unihelm_distro::Family::Debian, unihelm_distro::Family::Rhel] {
            let (distro, _) = unihelm_distro::mock::mock_distro_with_recorder(family);
            for component in [
