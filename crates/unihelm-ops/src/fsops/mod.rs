...
impl FsRunner {
    ...
    ///
    /// A helper-reported failure comes back as the mapped [`UnihelmError`], so
    ...
    pub async fn call(
        ...
        payload: Vec<u8>,
        timeout: Duration,
    ) -> unihelm_core::Result<(FsData, Vec<u8>)> {
        let call = FsCall {
        ...
        let (reply, reply_payload) = match self {
            FsRunner::Local => {
                ...
                match tokio::time::timeout(timeout, handle).await {
                    ...
                    Ok(joined) => joined
                        .map_err(|e| UnihelmError::internal(format!("fs task panicked: {e}")))?
                        .map_err(|e| {
                            UnihelmError::internal(format!("local fs helper failed: {e}"))
                        })?,
                }
            }
...
fn reply_error(kind: FsErrorKind, message: String) -> UnihelmError {
    let code = match kind {
...
/// Spawn `unihelm-agentd --fs-helper` for one request and speak the pipe
/// protocol with it.
///
...
async fn run_helper_process(
    ...
    payload: &[u8],
    timeout: Duration,
) -> unihelm_core::Result<(FsReply, Vec<u8>)> {
    ...
    let mut child = cmd
        .spawn()
        .map_err(|e| UnihelmError::internal(format!("could not spawn the fs helper: {e}")))?;

    let header = serde_json::to_vec(call)
        .map_err(|e| UnihelmError::internal(format!("unserialisable fs call: {e}")))?;
