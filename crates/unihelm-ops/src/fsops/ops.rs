...
use unihelm_core::{
    ErrorCode, LinuxUser, Permission, Result, SubscriptionId, TenantPath, UnihelmError,
};
    ...
    async fn run(&self, ctx: &OpContext, input: Self::Input) -> Result<Self::Output> {
        ...
        let FsData::Content {
            ...
            return Err(UnihelmError::internal(
                "the fs helper answered out of shape",
            ));
        };
        Ok(ReadOutput {
...
impl TypedOperation for Write {
    ...
    async fn run(&self, ctx: &OpContext, input: Self::Input) -> Result<Self::Output> {
        ...
        let content = BASE64.decode(&input.content_b64).map_err(|e| {
            UnihelmError::new(ErrorCode::InvalidInput, format!("content_b64: {e}"))
                .with_field("content_b64")
        })?;
...
async fn create_trash_as_root(ctx: &OpContext, fs: &TenantFs) -> Result<()> {
    ...
    if matches!(fs.runner, FsRunner::Tenant { .. }) {
        unihelm_distro::Cmd::new("chown")
            ...
            .await
            .map_err(|e| {
                UnihelmError::internal(format!("could not hand over the recycle bin: {e}"))
            })?;
    }

    std::fs::set_permissions(&path, std::os::unix::fs::PermissionsExt::from_mode(0o700))
        .map_err(|e| UnihelmError::internal(format!("could not lock down the recycle bin: {e}")))?;
...
mod tests {
    ...
    ) -> unihelm_core::Result<serde_json::Value> {
    ...
    async fn another_tenants_subscription_is_out_of_scope() {
        use unihelm_core::{Email, TenantScope, Username};
