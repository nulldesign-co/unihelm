...
use unihelm_db::alerts::{AlertEvent, AlertKind, AlertRule, ChannelKind, NotifyChannel};
use unihelm_distro::svc::{ManagedUnit, UnitState};
use unihelm_metrics::ServerSnapshot;
...
pub struct NotificationPayload {
    ...
    /// `raised` or `resolved`.
    pub state: String,
    /// RFC 3339, UTC — the panel's one clock (`unihelm_db::now`).
    pub at: String,
}
...
fn validate_telegram(config: &TelegramConfig) -> Result<()> {
    let bad_token = |detail: &str| {
        UnihelmError::new(ErrorCode::InvalidInput, detail.to_string())
            .with_field("config.bot_token")
    };
...
pub async fn evaluate_with(ctx: &OpContext, transport: &dyn Transport) -> Result<Vec<Raised>> {
    ...
    let rules = db.alert_rules().await.map_err(UnihelmError::from)?;
    if rules.is_empty() {
        return Ok(Vec::new());
    }

    let readings = collect(ctx, &rules).await?;
    let open = db.open_alert_events().await.map_err(UnihelmError::from)?;
...
fn validate_rule(kind: AlertKind, target: Option<&str>, threshold: Option<f64>) -> Result<f64> {
    let field = |detail: String, f: &'static str| {
        UnihelmError::new(ErrorCode::InvalidInput, detail).with_field(f)
    };
        ...
        let channel = match input.id {
            None => {
                let kind = input.kind.ok_or_else(|| {
                    UnihelmError::new(ErrorCode::InvalidInput, "a new channel needs a `kind`")
                        .with_field("kind")
                })?;
                let label = label.ok_or_else(|| {
                    UnihelmError::new(ErrorCode::InvalidInput, "a new channel needs a `label`")
                        .with_field("label")
                })?;
            ...
            Some(id) => {
                let existing = ctx
                    ...
                    .await
                    .map_err(UnihelmError::from)?
                    .ok_or_else(|| UnihelmError::not_found("notify channel"))?;
...
impl TypedOperation for ChannelsDelete {
    ...
    async fn run(&self, ctx: &OpContext, input: Self::Input) -> Result<Self::Output> {
        let deleted = ctx
            ...
            .map_err(UnihelmError::from)?;
        if !deleted {
            return Err(UnihelmError::not_found("notify channel"));
        }
...
mod tests {
    ...
    use unihelm_metrics::{CpuUsage, DiskUsage, LoadAverage, MemoryUsage, NetworkTotals};
    ...
    async fn evaluation_records_the_event_even_when_every_channel_fails() {
        ...
        ctx.distro()
            .svc
            .action(&nginx, unihelm_distro::svc::SvcAction::Start)
            .await
            .unwrap();
        ctx.distro()
            .svc
            .action(&nginx, unihelm_distro::svc::SvcAction::Stop)
            .await
            .unwrap();
    ...
    async fn rules_and_channels_are_admin_only() {
        ...
        for (op, input) in [
            ...
            assert_eq!(
                err.code,
                unihelm_core::ErrorCode::PermissionDenied,
