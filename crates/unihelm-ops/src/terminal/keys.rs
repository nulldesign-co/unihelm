...
use unihelm_core::{ErrorCode, LinuxUser, Permission, Result, Role, UnihelmError};
use unihelm_db::Db;
...
pub const BEGIN_MARKER: &str = "# ---- BEGIN UNIHELM-MANAGED KEYS ----";
pub const END_MARKER: &str = "# ---- END UNIHELM-MANAGED KEYS ----";
...
fn invalid(message: impl Into<String>) -> UnihelmError {
    UnihelmError::new(ErrorCode::InvalidInput, message).with_field("key")
}
...
fn find_block(lines: &[&str]) -> Result<BlockSpan> {
    ...
    let Some(begin) = begin else {
        ...
        if lines.iter().any(|l| l.trim() == END_MARKER) {
            return Err(UnihelmError::new(
                ...
                "authorized_keys has a Unihelm end marker with no begin marker; \
    ...
    let end = lines[begin + 1..]
        ...
        .ok_or_else(|| {
            UnihelmError::new(
                ErrorCode::ConfigDrift,
                "authorized_keys has an unterminated Unihelm block; the panel will not \
    ...
    async fn write(&self, ctx: &OpContext, existing: &str, keys: &[SshPublicKey]) -> Result<()> {
        ...
        self.runner
            ...
            .or_else(|e| match e.code {
                ...
                ErrorCode::PermissionDenied => Err(UnihelmError::new(
...
async fn ensure_can_manage_keys(db: &Db, role: Role, subscription: &Subscription) -> Result<()> {
        ...
        .map_err(UnihelmError::from)?
    {
        ...
        Some(plan) => Err(UnihelmError::new(
...
mod tests {
    ...
    async fn write_keys(home: &std::path::Path, existing: &str, keys: &[SshPublicKey]) {
        let target = Target {
            linux_user: LinuxUser::parse("uh_test").unwrap(),
