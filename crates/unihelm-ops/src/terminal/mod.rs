...
//! not the one being restarted, so `unihelm-agentd` owns the master fd, the
//! child, and the scrollback; `unihelm-web` holds nothing but a WebSocket and a
...
//! [`unihelm_distro::exec::reexec_current`], and the helper calls the *same*
//! `drop_privileges` in `unihelm-agentd`'s `main.rs` that `--fs-helper` and
...
use unihelm_core::{
    AuthContext, ErrorCode, LinuxUser, Permission, Result, Role, SubscriptionId, UnihelmError,
};
use unihelm_db::Db;
...
pub async fn authorize(
    ...
    match (auth.acting_role, target) {
        ...
        // condition inside it.
        (Role::Customer, TerminalTarget::Root) => Err(UnihelmError::new(
        ...
        (Role::Reseller, _) => Err(UnihelmError::new(
            ErrorCode::PermissionDenied,
...
pub(crate) async fn resolve_subscription(
    ...
    auth: &AuthContext,
    subscription_id: Option<i64>,
) -> Result<unihelm_db::subscriptions::Subscription> {
...
fn pick_root_shell() -> Result<PathBuf> {
    ALLOWED_SHELLS
        ...
        .ok_or_else(|| UnihelmError::internal("no usable login shell is installed on this server"))
}
...
mod tests {
    ...
    use unihelm_core::{Email, TenantScope, UserId, Username};
