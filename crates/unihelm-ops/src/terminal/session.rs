...
use unihelm_core::{AuthContext, ErrorCode, Result, UnihelmError, UserId};
use unihelm_db::Db;
use unihelm_db::audit::NewAuditEntry;
use unihelm_ipc::frame::TerminalTarget;
use uuid::Uuid;
...
impl TerminalRegistry {
    ...
    pub async fn for_owner(&self, id: Uuid, auth: &AuthContext) -> Result<Arc<SessionHandle>> {
        ...
        match sessions.get(&id) {
            ...
            _ => Err(UnihelmError::not_found("terminal session")),
        }
    }
    ...
    pub async fn close(&self, id: Uuid, auth: &AuthContext, reason: &str) -> Result<()> {
        ...
        if handle.info.owner != auth.actor_user_id {
            return Err(UnihelmError::not_found("terminal session"));
        }
...
impl PtySpawner for RealPtySpawner {
    async fn spawn(&self, plan: &SessionPlan, cols: u16, rows: u16) -> Result<SpawnedPty> {
        ...
        let pair = super::pty::open_pty(cols, rows)
            .map_err(|e| UnihelmError::internal(format!("could not allocate a pty: {e}")))?;
        ...
        let dup = |what: &'static str| -> Result<Stdio> {
            pair.slave
                .try_clone()
                .map(Stdio::from)
                .map_err(|e| UnihelmError::internal(format!("could not dup the pty {what}: {e}")))
        };
        cmd.stdin(dup("stdin")?)
            .stdout(dup("stdout")?)
            .stderr(dup("stderr")?)
        ...
        let mut child = cmd
            .spawn()
            .map_err(|e| UnihelmError::internal(format!("could not start the pty helper: {e}")))?;
        ...
        let reader_fd = pair
            .master
            .try_clone()
            .map_err(|e| UnihelmError::internal(format!("could not dup the pty master: {e}")))?;
        let writer_fd = pair
            .master
            .try_clone()
            .map_err(|e| UnihelmError::internal(format!("could not dup the pty master: {e}")))?;
        ...
        std::thread::Builder::new()
            ...
            .map_err(|e| UnihelmError::internal(format!("could not start the pty reader: {e}")))?;

        let (in_tx, in_rx) = std::sync::mpsc::channel::<Vec<u8>>();
        std::thread::Builder::new()
            .name(format!("unihelm-pty-w-{pid}"))
            .spawn(move || {
            ...
            .map_err(|e| UnihelmError::internal(format!("could not start the pty writer: {e}")))?;
...
mod tests {
    ...
    use unihelm_core::{Email, Role, SubscriptionId, TenantScope, Username};
    ...
    impl PtySpawner for FakeSpawner {
        async fn spawn(&self, _plan: &SessionPlan, _cols: u16, _rows: u16) -> Result<SpawnedPty> {
            ...
            if self.fail {
                return Err(UnihelmError::internal("no shell for you"));
            }
    ...
    fn the_helper_argv_names_no_program_and_no_shell_flag_for_root() {
        ...
        let tenant = SessionPlan::Tenant {
            uid: 5001,
            gid: 5001,
            linux_user: unihelm_core::LinuxUser::parse("uh_demo").unwrap(),
            subscription_id: SubscriptionId(1),
            home: PathBuf::from("/home/uh_demo"),
            shell: PathBuf::from("/bin/bash"),
        };
