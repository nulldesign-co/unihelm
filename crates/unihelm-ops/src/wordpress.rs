...
//! [`unihelm_distro::exec::reexec_current`], and the child calls
...
//! machinery (`unihelm-agentd --wp-helper`), not a second privilege-drop
//! mechanism: the re-exec, the uid/gid drop and its proof are literally the
//! same code in `crates/unihelm-agentd/src/main.rs`.
//!
...
use unihelm_core::{
    DbName, Domain, Email, ErrorCode, LinuxUser, Permission, Result, SiteId, TenantPath,
    UnihelmError,
};
use unihelm_db::databases::DbEngine;
use unihelm_db::sites::Site;
use unihelm_db::subscriptions::Subscription;
use unihelm_db::wordpress::{NewWpInstall, WpInstall};
...
///   release key pinned and checked through `unihelm_distro::pgp`, the way
...
/// `unihelm_distro::repos::UNVERIFIED_PINS`: it protects against a later
...
/// (`unihelm_ipc::client::DEFAULT_CALL_TIMEOUT`, 30 s). Twenty-five seconds
...
///
/// Every one of them is a shell metacharacter. Through [`unihelm_distro::Cmd`]
...
pub fn validate_arg(arg: &str) -> Result<&str> {
    ...
    if !arg.is_ascii() {
        ...
        return Err(
            UnihelmError::new(ErrorCode::InvalidInput, "WP-CLI arguments must be ASCII")
                .with_field("args"),
        );
    }
...
/// Spawn `unihelm-agentd --wp-helper` for one run and read its single-line JSON
/// reply.
///
...
/// ([`reexec_current`](unihelm_distro::exec::reexec_current) clears it) and
/// establishes its own.
async fn run_helper_process(
    ...
    let mut cmd = unihelm_distro::exec::reexec_current(&args).map_err(UnihelmError::from)?;
    ...
    let mut child = cmd
        .spawn()
        .map_err(|e| UnihelmError::internal(format!("could not spawn the WP-CLI helper: {e}")))?;
    ...
    match tokio::time::timeout(timeout, exchange).await {
        ...
        Ok(Err(e)) => Err(UnihelmError::internal(format!(
        ...
        Ok(Ok((reply, diagnostics, status))) => {
            ...
            serde_json::from_slice::<WpOutput>(&reply).map_err(|e| {
                UnihelmError::internal(format!("the WP-CLI helper answered out of shape: {e}"))
            })
        }
    }
}
...
///
/// Not `unihelm_config::managed::write_atomic`: that prepends the
/// UNIHELM-MANAGED comment header, and a phar is a signed archive whose bytes
...
async fn target_for_install(ctx: &OpContext, install_id: i64) -> Result<(WpInstall, WpTarget)> {
    let install = ctx
        ...
        .await
        .map_err(UnihelmError::from)?
        .ok_or_else(|| UnihelmError::not_found("WordPress installation"))?;
...
pub fn render_wp_config(cfg: &WpConfig<'_>) -> Result<String> {
    ...
    out.push_str("<?php\n");
    out.push_str("/**\n * WordPress configuration, generated by Unihelm (spec §11.12).\n");
...
mod tests {
    ...
    fn the_panel_decides_the_path_and_puts_it_first() {
        let argv = wp_argv(
            Path::new("/home/uh_x/sites/example.com/public"),
            vec!["core".into(), "version".into()],
        );
        assert_eq!(argv[0], "--path=/home/uh_x/sites/example.com/public");
    ...
    fn a_value_containing_spaces_stays_a_single_argv_element() {
        ...
        let argv = wp_argv(Path::new("/home/uh_x/sites/d/public"), args);
    ...
    fn an_unprivileged_agent_runs_locally_and_never_pretends_to_drop() {
        ...
        let runner = WpRunner::for_user(&LinuxUser::parse("uh_test").unwrap());
        if euid != 0 {
