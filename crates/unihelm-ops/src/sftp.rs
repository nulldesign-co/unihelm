...
//!
//! One managed drop-in, `/etc/ssh/sshd_config.d/50-unihelm.conf`, carries a
//! single `Match Group unihelm-sftp` block: `ChrootDirectory %h`,
...
//! (`unihelm_db::password`). So the hash is computed in-process as sha512-crypt
//! (the `sha-crypt` crate, pure Rust) and installed with `usermod --password`,
//! argv only: [`unihelm_distro::Cmd`] deliberately has no stdin plumbing
...
pub const SFTP_GROUP: &str = "unihelm-sftp";
...
impl TypedOperation for Enable {
    ...
    async fn run(&self, ctx: &OpContext, input: Self::Input) -> Result<Self::Output> {
        ...
        if let Some(password) = &input.password {
            unihelm_db::password::check_strength(password)?;
        }
...
fn ensure_plan_allows_sftp(subscription: &Subscription) -> Result<()> {
    match subscription.plan_id {
        None => Ok(()),
        Some(plan_id) => Err(UnihelmError::new(
...
/// write → validate → rollback (see `unihelm_config::apply` for why), so by
...
/// `groupadd --system -- unihelm-sftp`: a system group — no human ever logs in
/// *as* it, it only exists for the `Match` block to key on.
fn groupadd_argv() -> Vec<String> {
    vec!["--system".into(), "--".into(), SFTP_GROUP.into()]
}

/// `usermod --append --groups unihelm-sftp -- <user>`. `--append` is what makes
...
/// `gpasswd --delete <user> -- unihelm-sftp` — the inverse of the grant.
...
fn sftp_password_hash(password: &str) -> Result<String> {
    ...
    sha_crypt::ShaCrypt::default()
        ...
        .map_err(|e| UnihelmError::internal(format!("sha512-crypt failed: {e}")))
}
...
mod tests {
    ...
    fn the_password_argv_carries_a_hash_never_the_cleartext() {
        ...
        assert_eq!(
            argv,
            vec!["--password", "$6$salt$hash", "--", "uh_abc12345"]
        );
        assert!(
    ...
    fn the_home_is_surrendered_to_root_with_world_traverse() {
        let step = home_step(Path::new("/home/uh_abc12345"));
    ...
    fn a_missing_sites_directory_is_created_so_the_login_has_somewhere_writable() {
        ...
        assert_eq!(
            steps[0].owner, "uh_abc12345:uh_abc12345",
    ...
    fn a_corrupted_home_row_is_refused_before_any_chown() {
        ...
        let good = Subscription {
            ...
            home_dir: "/home/uh_abc12345".into(),
    ...
    async fn the_reloader_reloads_the_right_unit_per_family() {
        ...
        use unihelm_distro::mock::mock_distro_with_recorder;
        ...
        for (family, unit) in [
            ...
            distro
                .svc
                .action(
                    &unihelm_distro::svc::UnitName::parse(unit).unwrap(),
                    SvcAction::Start,
                )
                .await
                .unwrap();
    ...
    async fn a_tenant_cannot_toggle_sftp_on_someone_elses_subscription() {
        ...
        use std::sync::Arc;
        use unihelm_core::{AuthContext, Role, TenantScope};
