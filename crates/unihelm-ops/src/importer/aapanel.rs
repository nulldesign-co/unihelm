...
//!    domains. Unihelm's vhosts cannot take over a hostname while another
...
pub async fn plan(
    ...
    plan.note(
        "aaPanel is still installed on this server and still owns its own nginx configuration. \
         Unihelm's vhosts cannot serve these domains until aaPanel's are removed or its nginx is \
...
pub async fn inventory_fingerprint(root: &Path) -> Result<String> {
    ...
    hasher.update(b"unihelm-aapanel-inventory-v1");
    for site in &sites {
        hasher.update(b"\nsite\x00");
...
fn map_site(
    ...
    let Ok(primary) = Domain::parse(&site.name) else {
        plan.unmapped(
            UnmappedKind::Site,
            site.name.clone(),
            "aaPanel's site name is not a domain Unihelm will serve (an IP-only or wildcard site)",
        );
        return;
    };
...
/// shared between sites and Unihelm's ownership is the subscription anyway.
...
async fn report_unmappables(plan: &mut ImportPlan, conn: &mut SqliteConnection) {
    // FTP accounts: aaPanel manages its own; Unihelm's SFTP is per-subscription
        ...
        for row in rows {
            if let Ok(name) = row.try_get::<String, _>("name") {
                plan.unmapped(
                    UnmappedKind::Ftp,
                    name,
                    "Unihelm has no per-site FTP accounts; the tenant gets one chrooted SFTP \
    ...
    // `/www/server/panel/vhost/cert`. Unihelm issues its own.
    plan.unmapped(
        UnmappedKind::Certificate,
        "aaPanel-issued certificates",
        "certificates are not imported; Unihelm issues its own from Let's Encrypt once each \
