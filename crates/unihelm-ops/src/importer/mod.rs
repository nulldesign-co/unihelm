...
//! than for `unihelm` on a terminal.
...
use unihelm_core::{
    DbName, Domain, ErrorCode, Permission, PhpVersion, Result, SubscriptionId, UnihelmError,
};
use unihelm_db::databases::DbEngine;
use unihelm_db::imports::{ImportPlanRecord, ImportSource, NewImportPlan};
use unihelm_db::subscriptions::Subscription;
use unihelm_distro::CmdOutput;
...
fn validated_source_path(raw: &str) -> Result<PathBuf> {
    ...
    if trimmed.is_empty() {
        return Err(
            UnihelmError::new(ErrorCode::InvalidPath, "the source path is empty")
                .with_field("path"),
        );
    }
    ...
    if !path.is_absolute() {
        return Err(
            UnihelmError::new(ErrorCode::InvalidPath, "the source path must be absolute")
                .with_field("path"),
        );
    }
    if path
    ...
    match source {
        ImportSource::Cpanel => {
            ...
            tokio::task::spawn_blocking(move || fingerprint(&path))
                .await
                .map_err(|e| UnihelmError::internal(format!("fingerprint task panicked: {e}")))?
        }
...
pub struct PlanInput {
    ...
    /// unknown or is one Unihelm does not offer.
    #[serde(default)]
    ...
    async fn run(&self, ctx: &OpContext, input: Self::Input) -> Result<Self::Output> {
        ...
        let plan = match source_kind {
            ImportSource::Cpanel => {
                ...
                .await
                .map_err(|e| UnihelmError::internal(format!("scan task panicked: {e}")))??
            }
        ...
        let plan_json = serde_json::to_string(&plan)
            .map_err(|e| UnihelmError::internal(format!("the plan will not serialise: {e}")))?;
        ...
        let records = match input.plan_id {
            Some(id) => vec![
                repo.by_id(id)
                    .await
                    .map_err(UnihelmError::from)?
                    .ok_or_else(|| UnihelmError::not_found("import plan"))?,
            ],
            None => repo
...
fn summarise(record: ImportPlanRecord, detailed: bool) -> PlanSummary {
    ...
    PlanSummary {
        ...
        created_at: unihelm_db::to_sql_time(record.created_at),
        applied_at: record.applied_at.map(unihelm_db::to_sql_time),
...
fn next_steps(plan: &ImportPlan, outcome: &ApplyOutcome) -> Vec<String> {
    ...
    if plan.source == ImportSource::Aapanel {
        steps.push(
            "Remove aaPanel's vhosts (or stop its nginx) before Unihelm can serve these domains"
                .to_string(),
        );
    }
...
async fn create_site(
    ...
    for alias in &planned.aliases {
        ...
        match ctx
            .db()
            .sites(ctx.scope())
            .add_alias(unihelm_core::SiteId(created.site_id), &alias, false)
            .await
        {
            Ok(_) => {
                added += 1;
...
async fn copy_files(
    ...
    // cannot collide.
    let staging_name = format!(".unihelm-import-{}-{index}.tar.gz", record.id);
    let (parent, name) = safepath::resolve_new(&home, Path::new(&staging_name))
        .map_err(|e| UnihelmError::new(ErrorCode::InvalidPath, e.message))?;
    ...
    let build = {
        ...
        tokio::task::spawn_blocking(move || build_staging_archive(&files, &source_path, &staged))
            .await
            .map_err(|e| UnihelmError::internal(format!("staging task panicked: {e}")))?
    };
            ...
            let extract = crate::fsops::ops::Extract
                .run(
                    ...
                    crate::fsops::ops::ExtractInput {
                        subscription_id: Some(subscription.id.get()),
                        archive: unihelm_core::TenantPath::parse(&staging_name)?,
...
fn tenant_relative_docroot(
    ...
    planned: &PlannedSite,
    subscription: &Subscription,
) -> Result<unihelm_core::TenantPath> {
    ...
    unihelm_core::TenantPath::parse(&relative.to_string_lossy())
}
...
fn build_staging_archive(
    ...
    match files {
        ...
        FileSource::Directory { path } => {
            ...
            .map_err(|e| UnihelmError::new(ErrorCode::Internal, e.message))?;
...
/// persist (`unihelm_ops::db`'s module docs spell out why that rules out a
...
async fn load_data(
    ...
    let sql = match &planned.payload {
        DumpSource::TarMember { path } => {
            ...
            .await
            .map_err(|e| UnihelmError::internal(format!("dump read panicked: {e}")))??
        }
...
/// shape as `unihelm_ops::db::DbShell`, and for the same reason.
#[async_trait]
...
impl SqlLoader for SystemLoader {
    async fn run(&self, job: &LoadJob) -> Result<CmdOutput> {
        let (program, args) = job
            .argv
            .split_first()
            .ok_or_else(|| UnihelmError::internal("a LoadJob must carry a program"))?;
...
async fn dump_local_database(
    ...
    let output = unihelm_distro::Cmd::new("mariadb-dump")
        .args([
...
fn mysql_socket(ctx: &OpContext) -> &'static str {
    match ctx.distro().info.family {
        unihelm_distro::Family::Debian => "/run/mysqld/mysqld.sock",
        unihelm_distro::Family::Rhel => "/var/lib/mysql/mysql.sock",
    }
}
...
mod tests {
    ...
    use unihelm_db::imports::ImportSource as Src;
    ...
    fn the_dump_client_reads_its_credentials_from_a_file_never_from_the_argv() {
        let argv = mysql_load_argv(Path::new("/var/lib/unihelm/state/import/wp.cnf"), "wp");
        assert_eq!(argv[0], "mariadb");
        assert_eq!(
            argv[1], "--defaults-file=/var/lib/unihelm/state/import/wp.cnf",
    ...
    fn the_document_root_is_addressed_relative_to_the_tenant_home() {
        let subscription = unihelm_db::subscriptions::Subscription {
            id: SubscriptionId(1),
            customer_id: unihelm_core::UserId(1),
            plan_id: None,
            linux_user: "uh_abc".into(),
            home_dir: "/home/uh_abc".into(),
            status: unihelm_db::SubscriptionStatus::Active,
        ...
        let relative =
            tenant_relative_docroot(Path::new("/home/uh_abc"), &planned, &subscription).unwrap();
