...
use sha2::Sha256;
use unihelm_core::{ErrorCode, Permission, Result, UnihelmError, UserId};
...
/// `X-Unihelm-Signature: v1=<hex>`.
pub const SIGNATURE_HEADER: &str = "X-Unihelm-Signature";
/// `X-Unihelm-Timestamp: <unix seconds>` — inside the signed string, so it
/// cannot be rewritten by anyone who did not have the secret.
pub const TIMESTAMP_HEADER: &str = "X-Unihelm-Timestamp";
/// `X-Unihelm-Event: <event name>`.
pub const EVENT_HEADER: &str = "X-Unihelm-Event";
/// `X-Unihelm-Delivery: <delivery id>` — stable across retries of the same
/// delivery, which is what makes at-least-once safe to consume.
pub const DELIVERY_HEADER: &str = "X-Unihelm-Delivery";
...
pub struct SignedRequest {
    ...
    pub delivery_id: i64,
    pub timestamp: i64,
    /// The full `X-Unihelm-Signature` value, `v1=<hex>`.
...
pub async fn emit(ctx: &OpContext, event: &str, data: serde_json::Value) -> usize {
    ...
    let at = unihelm_db::to_sql_time(unihelm_db::now());
    let mut queued = 0;
    for hook in hooks {
        ...
        match ctx
            .db()
            .enqueue_delivery(hook.id, event, &placeholder, unihelm_db::now())
            .await
        {
            Ok(id) => {
...
pub async fn deliver_due(ctx: &OpContext, transport: &dyn Deliverer) -> Result<String> {
    ...
    for item in due {
        ...
        let retry_at = backoff(item.delivery.attempts + 1).map(|d| unihelm_db::now() + d);
        let streak = db
            .delivery_failed(
                item.delivery.id,
                item.delivery.webhook_id,
...
impl From<unihelm_db::Webhook> for WebhookView {
    fn from(w: unihelm_db::Webhook) -> Self {
        Self {
            id: w.id,
            ...
            events: w.events,
            active: w.active,
            last_delivery_at: w.last_delivery_at.map(unihelm_db::to_sql_time),
...
pub struct ListOutput {
    ...
    pub deliveries: Option<Vec<unihelm_db::WebhookDelivery>>,
}
...
impl TypedOperation for List {
    ...
    async fn run(&self, ctx: &OpContext, input: Self::Input) -> Result<Self::Output> {
        ...
        let webhooks = repo.list().await.map_err(UnihelmError::from)?;
        ...
        let deliveries = match input.id {
            Some(id) => {
                ...
                if !webhooks.iter().any(|w| w.id == id) {
                    return Err(UnihelmError::not_found("webhook"));
                }
                Some(
                    ctx.db()
        ...
        Ok(ListOutput {
            ...
            max_per_owner: unihelm_db::webhooks::MAX_HOOKS_PER_OWNER,
            deliveries,
        })
    }
}
...
const SCHEME_HINT: &str = "HMAC-SHA256 over `v1:<X-Unihelm-Timestamp>:<raw body>`, \
                           compared against X-Unihelm-Signature (see docs/webhooks.md)";
        ...
        match input.id {
            Some(id) => {
                ...
                let sealed = match &secret {
                    Some(s) => Some(ctx.master_key().seal_str(s).map_err(UnihelmError::from)?),
                    None => None,
                };
                let hook = repo
...
impl Set {
    ...
    async fn resolve_owner(&self, ctx: &OpContext, requested: Option<i64>) -> Result<UserId> {
        ...
        ctx.db()
            ...
            .await
            .map_err(UnihelmError::from)?
            .ok_or_else(|| UnihelmError::not_found("account"))?;
        Ok(id)
    }
}
...
impl TypedOperation for Test {
    ...
    async fn run(&self, ctx: &OpContext, input: Self::Input) -> Result<Self::Output> {
        ...
        let body = serde_json::to_string(&EventEnvelope {
            ...
            id: 0,
            at: unihelm_db::to_sql_time(unihelm_db::now()),
            data: serde_json::json!({ "panel": "unihelm", "hook_id": hook.id }),
        })
        .map_err(|e| UnihelmError::internal(format!("test payload: {e}")))?;
...
mod tests {
    ...
    async fn a_dead_endpoint_is_disabled_instead_of_retried_forever() {
        ...
        for _ in 0..FAILURE_THRESHOLD {
            ...
            .bind(unihelm_db::to_sql_time(
                unihelm_db::now() - time::Duration::hours(1),
            ))
            .execute(db.pool())
            .await
            .unwrap();
