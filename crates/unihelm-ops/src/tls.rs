...
use unihelm_core::{Result, UnihelmError};
...
pub fn write_self_signed(dir: &Path, names: &[String]) -> Result<()> {
    ...
    let key = KeyPair::generate()
        .map_err(|e| UnihelmError::internal(format!("could not generate a key: {e}")))?;

    let mut params = CertificateParams::new(names.clone())
        .map_err(|e| UnihelmError::internal(format!("invalid certificate names: {e}")))?;

    // A subject that says what this is, so an operator who inspects it in a
    // browser sees "Unihelm default" rather than a blank certificate they have to
    // guess about.
    let mut dn = DistinguishedName::new();
    dn.push(DnType::CommonName, names[0].clone());
    dn.push(DnType::OrganizationName, "Unihelm (self-signed)");
    ...
    let cert = params
        .self_signed(&key)
        .map_err(|e| UnihelmError::internal(format!("could not self-sign: {e}")))?;
...
fn write_with_mode(path: &Path, bytes: &[u8], mode: u32) -> Result<()> {
    ...
    write()
        .map_err(|e| UnihelmError::internal(format!("could not write {}: {e}", temp.display())))?;
