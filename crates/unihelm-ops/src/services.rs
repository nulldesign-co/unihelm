//! Bridging `unihelm-config`'s apply engine to the real services.
//!
...
use unihelm_config::apply::{NewRevision, Reloader, RevisionStore, StoredRevision, Validator};
...
impl FpmValidator {
    pub fn new(distro: &Distro, version: PhpVersion) -> Self {
        Self {
            binary: unihelm_config::paths::fpm_binary(distro.info.family, version),
        }
    }
}
...
mod tests {
    ...
    async fn revisions_round_trip_through_the_database() {
        ...
        let id = store
            .record(NewRevision {
                path: "/etc/nginx/unihelm.d/site-a.conf".into(),
                sha256: "abc".into(),
                content: "server {}".into(),
        ...
        let active = store
            .active("/etc/nginx/unihelm.d/site-a.conf")
            .await
            .unwrap()
            .unwrap();
        ...
        assert!(
            store
                .active("/etc/nginx/unihelm.d/nothing.conf")
                .await
                .unwrap()
                .is_none()
        );
