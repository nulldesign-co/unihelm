# FERRUM — Modern Multi-Tenant Hosting Control Panel
...
> Codename `ferrum` (Latin for iron — the thing Rust grows on) is a placeholder; renaming it is a find-and-replace, do not block on it.
...
| Pain (observed) | Where | Ferrum's design answer |
|---|---|---|
...
|---|---|---|
| Panel idle RSS (ferrum-web + ferrum-agentd combined) | ≤ 80 MB (target 50 MB) | integration harness reads `/proc/*/smaps_rollup` after 5 min idle; CI fails over budget |
...
| CLI | `clap` v4 (`ferrum` binary) | talks to the same REST API over UDS |
...
```
/usr/local/ferrum/bin/{ferrum-web, ferrum-agentd, ferrum}   # binaries
/etc/ferrum/config.toml                                     # minimal bootstrap config
/var/lib/ferrum/panel.db                                    # SQLite (0600 root)
/var/lib/ferrum/state/                                      # acme accounts, rendered configs, task logs
/run/ferrum/agent.sock                                      # UDS, 0700, owner ferrum-web user
/var/log/ferrum/                                            # file logs (also journald)
...
**Two processes, hard privilege boundary.** `ferrum-web` runs as an unprivileged system user and can be exposed to the internet. `ferrum-agentd` runs as root, listens ONLY on a Unix socket with strict peer-credential checks (SO_PEERCRED must match the ferrum user), and executes only operations registered in its whitelist.
...
### 5.3 IPC protocol (`ferrum-ipc` crate)

Length-prefixed JSON frames over UDS (debuggable with `ferrum dev ipc-tap`); envelope:
...
Keep `ferrum-ipc` transport-abstract (`trait FrameTransport`), so a later `mTLS TcpTransport` turns remote agents into a feature, not a rewrite. No other clustering work in v1.
...
- `ferrum-web` unit hardening: `NoNewPrivileges`, `ProtectSystem=strict`, `PrivateTmp`, `CapabilityBoundingSet=`, RW only its state dirs.
...
- Self-monitoring: agentd watches web (and vice versa via socket ping); a `ferrum doctor` CLI command prints a full health report (units, sockets, db integrity, disk space, cert expiries).
...
| Layer | Mechanism |
|---|---|
| Identity | one Linux user per customer subscription (`ft_<short-id>`), no shell by default (`/usr/sbin/nologin`; `can_ssh` flips to bash with chroot-less but quota'd home) |
...
| CPU/RAM | one systemd slice per tenant (`ferrum-<user>.slice`) with `MemoryMax`, `CPUQuota`, `TasksMax` from plan; FPM pools and Node app units are placed in the tenant slice |
| Network | per-site nginx rate-limit/conn-limit toggles; bandwidth accounting from access logs (§11.11) |
| SFTP | openssh `Match Group ferrum-sftp` → `ChrootDirectory %h`, `ForceCommand internal-sftp` (panel manages an include file in sshd_config.d) |
...
### 7.2 `ferrum-distro` crate — the only place OS differences live
...
Encryption at rest for secrets columns (dns creds, smtp passwords, acme keys): libsodium sealed boxes with a master key in `/etc/ferrum/secret.key` (0600, generated at install).
...
1. Files Ferrum fully owns (rendered vhosts, fpm pools, unit files) live in dedicated include dirs (`/etc/nginx/ferrum.d/…`) and carry a `# FERRUM-MANAGED sha256:<hash>` header.
...
- Restore: browse snapshots → restore whole subscription, single site, single DB, or single files (to original or alternate path + "download as archive"); **disaster recovery**: fresh server + `ferrum restore --from <repo>` rebuilds panel state, then re-provisions stack + tenants (this is the ultimate integration test).
...
Curated JSON manifest catalog (in-repo, remotely updatable index): app slug, versions, compose template, required resources, exposed port, health path, env schema (typed prompts), backup paths. Install flow: pick subdomain → panel renders compose to `/var/lib/ferrum/apps/<tenant>/<app>` → up → nginx vhost + SSL → health gate. Lifecycle: upgrade (image pin bump with pre-snapshot), stop/start, logs, uninstall (with volume keep/delete choice). v1 catalog: phpMyAdmin, pgAdmin, Uptime Kuma, n8n, Ghost, Gitea, Metabase, code-server.
...
### 11.20 CLI (`ferrum`)
Full automation parity: `ferrum site create`, `ferrum php install 8.4`, `ferrum backup run <sub>`, `ferrum doctor`, `ferrum tenant suspend <id>`, `ferrum db mysql create …`, `ferrum task logs <id> -f`. Talks to the same REST API over the UDS with an admin token; ships shell completions. This is also how power users and the migration tooling script bulk operations.
...
1. **Privilege separation (§5).** `ferrum-web` never runs as root and never spawns privileged processes; all privileged work crosses the UDS into `ferrum-agentd`. Peer credentials (`SO_PEERCRED`) on the socket must match the `ferrum` user or the frame is rejected.
...
4. **Authorization checked twice.** `ferrum-web` authorizes via RBAC; `ferrum-agentd` re-checks the same tables against the propagated auth context (defense in depth). The repository layer takes a `TenantScope`, making an un-scoped tenant query impossible to write by accident.
...
6. **Secrets at rest are encrypted.** ACME account keys, DNS API creds, SMTP passwords, backup repo keys → libsodium sealed boxes under a master key in `/etc/ferrum/secret.key` (0600, generated at install). Nothing sensitive is logged; secret fields are masked in API responses and audit rows.
...
Repo/workspace layout (§15), the two daemons booting under systemd, `ferrum-ipc` with 2–3 trivial ops (ping, `svc.status`, `metrics.snapshot`), SQLite + first migrations, auth (login, argon2, sessions, one admin user via installer), RBAC scaffolding, the operation-registry pattern + the CI gates (no-`sh -c`, RSS budget, binary size), `ferrum-distro` traits with Apt+Dnf+Systemd implemented, base installer script + preflight, embedded React shell with login + empty dashboard + task drawer + i18n(en/fa)/RTL/theming wired. **Exit:** you can log in on Debian 13 and AlmaLinux 10 and see live server metrics; CI enforces budgets.
...
Plugin system (sidecar model), webhooks maturity, public API stability guarantee + versioning, `mTLS TcpTransport` groundwork for future multi-server, marketplace for app manifests. **Exit:** third parties can extend Ferrum without patching the core.
...
├─ crates/
│  ├─ ferrum-core/            # domain types, newtypes, RBAC, plan math, error taxonomy
│  ├─ ferrum-db/              # sqlx models, migrations, repositories (TenantScope-based)
│  ├─ ferrum-ipc/             # frame protocol, transport trait, client + server halves
│  ├─ ferrum-distro/          # PkgBackend/SvcBackend/FwBackend/SecModule + impls
│  ├─ ferrum-ops/             # the operation registry: each privileged op + typed input
│  ├─ ferrum-config/          # minijinja templates + render/validate/activate/rollback engine
│  ├─ ferrum-metrics/         # collector + rollups
│  ├─ ferrum-backup/          # rustic wrapper
│  ├─ ferrum-web/             # axum app, REST API, SSE, auth, embedded UI  (BINARY, unprivileged)
│  ├─ ferrum-agentd/          # root daemon: registry executor, task queue, scheduler (BINARY, root)
│  └─ ferrum-cli/             # `ferrum` (BINARY)
├─ ui/                        # React/TS/Vite app → built into ferrum-web via rust-embed
...
Testing expectations: unit tests in every crate; `ferrum-ops` gets a mock distro backend so ops are testable without root; integration harness runs the real thing in throwaway containers/VMs per distro; every bug fixed gets a regression test.
...
2. **Security invariants are non-negotiable:** no `sh -c`/string-interpolated commands anywhere; all privileged work goes through `ferrum-ops` with typed, validated inputs; web process never runs as root; agentd re-checks authorization; tenant fs ops run as the tenant uid.
...
6. **Distro differences live ONLY in `ferrum-distro`.** Modules must be OS-agnostic. Test on both families (SELinux enforcing on RHEL) before calling a feature done.
